...
export default function PublicDisplay() {
  ...
  return <main dir={locale === "ar" ? "rtl" : "ltr"} className="relative min-h-screen min-h-[100dvh] select-none overflow-hidden bg-[#07111f] text-white [touch-action:manipulation]">
    ...
    {displayImage ? <><div className="absolute inset-0 scale-105 bg-cover bg-center opacity-65 blur-2xl" style={{ backgroundImage: `url(${displayImage})` }} /><div className="absolute inset-0 bg-[radial-gradient(circle_at_25%_20%,#e76f3c66,transparent_40%),radial-gradient(circle_at_80%_75%,#2dd4bf44,transparent_35%)]" /><img key={`${slide?.id ?? index}-image`} src={displayImage} alt={displayTitle} className="nfood-display-media-enter absolute inset-0 h-full w-full object-contain p-8 brightness-110 contrast-110 saturate-125 drop-shadow-2xl sm:p-12" /></> : <div className="absolute inset-0 bg-[radial-gradient(circle_at_25%_20%,#e76f3c66,transparent_40%),radial-gradient(circle_at_80%_75%,#2dd4bf44,transparent_35%)]"><div className="absolute inset-0 grid place-items-center"><div className="max-w-xl px-8 text-center"><div className="mx-auto h-1.5 w-24 rounded-full bg-orange-300" /><p className="mt-8 text-xs font-black tracking-[0.3em] text-orange-200">NASSER CAFE · شاشة النكهات</p><h2 className="mt-5 text-4xl font-black text-white drop-shadow-2xl sm:text-6xl">{displayTitle || "اختيارك يستحق الظهور"}</h2><p className="mt-4 text-base text-slate-200 sm:text-xl">صورة الصنف قيد الاعتماد · العرض مستمر بجودة واضحة</p></div></div></div>}
    <div className="absolute inset-0 bg-gradient-to-br from-[#07111f]/60 via-[#07111f]/15 to-[#07111f]/80" />
    {activePlayback?.screen.qrEnabled && displayQrUrl && <div className={`absolute z-10 rounded-2xl p-3 shadow-2xl ring-1 ring-white/20 ${qrPositionClass}`} style={{ backgroundColor: activePlayback.screen.qrBackground }}><QRCodeSVG value={displayQrUrl} size={activePlayback.screen.qrSize} level="H" includeMargin fgColor={activePlayback.screen.qrForeground} bgColor={activePlayback.screen.qrBackground} /><p className="mt-2 text-center text-[10px] font-black text-white/80">{match ? "امسح للاستفادة من عرض المباراة" : "امسح لفتح المنيو"}</p></div>}
