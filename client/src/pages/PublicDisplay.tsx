...
import { displayVideoAttributes, getDisplayLayoutCount, isSplitDisplayLayout } from "@/lib/displayVideoSupport";
...
export default function PublicDisplay() {
  ...
  const isSplitLayout = isSplitDisplayLayout(activePlayback?.screen.displayLayout);
  const layoutCount = getDisplayLayoutCount(activePlayback?.screen.displayLayout);
  const visibleSlides = Array.from({ length: layoutCount }, (_, offset) => slides[(index * layoutCount + offset) % Math.max(slides.length, 1)]).filter(Boolean);
  ...
  const displayVideo = match ? "" : (slide?.externalVideoUrl ?? "");
  // الفيديو المستقل لا يتبع slide index؛ يبقى مستمرًا أثناء تبديل الصور.
  const continuousVideoSlide = match ? null : (slides.find((item) => Boolean(item?.externalVideoUrl)) ?? null);
  const continuousVideo = continuousVideoSlide?.externalVideoUrl ?? displayVideo;
  const continuousVideoPoster = continuousVideoSlide?.externalImageUrl ?? continuousVideoSlide?.mediaFile?.publicUrl ?? continuousVideoSlide?.menuItem?.imageUrl ?? displayImage;
  const hasVisibleMedia = Boolean(displayImage || continuousVideo || visibleSlides.some((item) => Boolean(item?.externalImageUrl ?? item?.mediaFile?.publicUrl ?? item?.menuItem?.imageUrl ?? item?.externalVideoUrl)));
  ...
  const qrSize = Math.min(Number(activePlayback?.screen.qrSize ?? 150), 150);
  const qrForeground = activePlayback?.screen.qrForeground === "#ffffff" ? "#111827" : (activePlayback?.screen.qrForeground ?? "#111827");
  const qrBackground = activePlayback?.screen.qrBackground === "#111c2e" ? "#ffffff" : (activePlayback?.screen.qrBackground ?? "#ffffff");
  const displayClock = useMemo(() => new Intl.DateTimeFormat(locale === "ar" ? "ar-SA" : "en-US", { dateStyle: "medium", timeStyle: "short" }).format(new Date(clockNow)), [clockNow, locale]);
  ...
  useEffect(() => { setVideoFailed(false); }, [continuousVideo]);
  ...
  return <main dir={locale === "ar" ? "rtl" : "ltr"} className="nfood-display-root relative min-h-screen min-h-[100dvh] select-none overflow-hidden bg-[#07111f] text-white [touch-action:manipulation]">
    ...
    {hasVisibleMedia ? isSplitLayout ? <div dir="ltr" className="absolute inset-0 grid grid-cols-2 gap-3 bg-[#07111f] p-3"><div className="relative min-h-0 overflow-hidden rounded-2xl bg-black">{continuousVideo && !videoFailed ? <video key={`continuous-video-${continuousVideo}`} src={continuousVideo} {...displayVideoAttributes(true)} autoPlay loop muted playsInline preload="auto" poster={continuousVideoPoster || undefined} aria-label="فيديو مستمر" className="nfood-display-media-enter h-full w-full object-cover" onError={() => setVideoFailed(true)} /> : <img src={displayImage} alt="محتوى الحملة" className="h-full w-full object-cover" onError={() => setImageFailed(true)} />}</div><div className="relative min-h-0 overflow-hidden rounded-2xl bg-[#07111f]">{visibleSlides[0] && (() => { const item = continuousVideoSlide ?? visibleSlides[0]; const imageUrl = item?.externalImageUrl ?? item?.mediaFile?.publicUrl ?? item?.menuItem?.imageUrl ?? continuousVideoPoster ?? displayImage; return <><img src={imageUrl} alt={item.title ?? item.menuItem?.name ?? displayTitle} className="nfood-display-media-enter nfood-display-media-fade h-full w-full object-cover brightness-110 contrast-110 saturate-125" onError={() => setImageFailed(true)} /><div dir="rtl" className="absolute inset-x-6 bottom-8 text-right text-white drop-shadow-2xl"><h2 className="text-3xl font-black sm:text-5xl">{item.title ?? item.menuItem?.name ?? displayTitle}</h2><p className="mt-2 text-base font-bold sm:text-xl">{item.subtitle ?? item.menuItem?.description ?? displaySubtitle}</p></div></>})()}</div></div> : <><div className="absolute inset-0 scale-105 bg-cover bg-center opacity-65 blur-2xl" style={{ backgroundImage: `url(${displayImage})` }} /><div className="absolute inset-0 bg-[radial-gradient(circle_at_25%_20%,#e76f3c66,transparent_40%),radial-gradient(circle_at_80%_75%,#2dd4bf44,transparent_35%)]" /><div className={`absolute inset-0 grid gap-2 p-0 ${layoutCount === 1 ? "grid-cols-1" : layoutCount === 2 ? "grid-cols-2" : layoutCount === 3 ? "grid-cols-3" : "grid-cols-2 grid-rows-2"}`}>{visibleSlides.map((item, slot) => { const imageUrl = item?.externalImageUrl ?? item?.mediaFile?.publicUrl ?? item?.menuItem?.imageUrl ?? displayImage; const videoUrl = slot === 0 ? continuousVideo : ""; return videoUrl && !videoFailed ? <video key={`continuous-video-${videoUrl}`} src={videoUrl} {...displayVideoAttributes(true)} autoPlay loop muted playsInline preload="auto" poster={continuousVideoPoster || imageUrl || undefined} aria-label="فيديو مستمر" className="nfood-display-media-enter nfood-display-media-fade h-full w-full min-h-0 object-cover brightness-110 contrast-110 saturate-125" onError={() => setVideoFailed(true)} /> : <img key={`${item?.id ?? index}-${slot}-image`} src={imageUrl} alt={item?.title ?? item?.menuItem?.name ?? displayTitle} onError={() => setImageFailed(true)} className="nfood-display-media-enter nfood-display-media-fade nfood-display-image h-full w-full min-h-0 object-cover brightness-110 contrast-110 saturate-125" />; })}</div></> : <div className="absolute inset-0 bg-[radial-gradient(circle_at_25%_20%,#e76f3c66,transparent_40%),radial-gradient(circle_at_80%_75%,#2dd4bf44,transparent_35%)]"><div className="absolute inset-0 grid place-items-center"><div className="max-w-xl px-8 text-center"><div className="mx-auto h-1.5 w-24 rounded-full bg-orange-300" /><p className="mt-8 text-xs font-black tracking-[0.3em] text-orange-200">NASSER CAFE · شاشة النكهات</p><h2 className="mt-5 text-4xl font-black text-white drop-shadow-2xl sm:text-6xl">{displayTitle || "اختيارك يستحق الظهور"}</h2><p className="mt-4 text-base text-slate-200 sm:text-xl">صورة الصنف قيد الاعتماد · العرض مستمر بجودة واضحة</p></div></div></div>}
    <div className="absolute inset-0 bg-gradient-to-br from-[#07111f]/60 via-[#07111f]/15 to-[#07111f]/80" />
    {activePlayback?.screen.qrEnabled && displayQrUrl && <div className={`nfood-display-qr absolute z-20 rounded-2xl p-3 shadow-2xl ring-1 ring-white/20 ${qrPositionClass}`} style={{ backgroundColor: qrBackground }}><QRCodeSVG value={displayQrUrl} size={qrSize} level="H" includeMargin fgColor={qrForeground} bgColor={qrBackground} /><p className="mt-2 text-center text-[10px] font-black text-white/80">{match ? "امسح للاستفادة من عرض المباراة" : "امسح لفتح المنيو"}</p><div className="nfood-display-promo-ribbon mt-3 min-w-[180px] max-w-[230px] rounded-xl border border-orange-200/30 bg-[#e76f3c]/90 px-3 py-2 text-center text-white shadow-lg"><p className="text-[9px] font-black uppercase tracking-[0.12em] text-orange-100">عروض وخصومات ناصر</p><p className="mt-1 truncate text-[11px] font-black">{slide?.badgeText || "اختيارات اليوم"}</p><p className="mt-1 truncate text-[10px] font-bold text-orange-50">{displayTitle || "أصناف تستحق التجربة"}</p></div></div>}
    <div className="relative flex min-h-screen min-h-[100dvh] flex-col justify-between p-4 sm:p-8 md:p-14">
      <header className="nfood-display-topbar"><div className="nfood-display-brand"><p>NFOOD DISPLAY</p></div><div className="nfood-display-cafe" title={displayName}>{displayName}</div><div className="nfood-display-status"><div className="flex items-center gap-2 rounded-full border border-emerald-300/25 bg-emerald-400/15 px-3 py-2 text-xs font-bold text-emerald-100" aria-label="الصوت مفعّل تلقائيًا"><Volume2 className="h-4 w-4 text-emerald-200" /> الصوت مفعّل تلقائيًا</div>{match && <div className="rounded-full bg-rose-500 px-4 py-2 text-xs font-black text-white">● وضع المباراة</div>}{matchSeconds !== null && <div className="rounded-full border border-amber-300/40 bg-amber-300/15 px-4 py-2 text-xs font-black text-amber-200">ينتهي خلال {Math.floor(matchSeconds / 60)}:{String(matchSeconds % 60).padStart(2, "0")}</div>}<div className={`flex items-center gap-2 rounded-full border px-4 py-2 text-xs font-bold ${connectionState === "connected" ? "border-emerald-300/20 bg-emerald-400/15 text-emerald-200" : connectionState === "connecting" ? "border-amber-300/20 bg-amber-300/15 text-amber-200" : "border-rose-300/20 bg-rose-400/15 text-rose-200"}`}>{connectionState === "connected" ? <Wifi className="h-4 w-4" /> : <WifiOff className="h-4 w-4" />} {connectionState === "connected" ? "متصل" : connectionState === "connecting" ? "جارٍ إعادة الاتصال" : "غير متصل · المحاولة مستمرة"}</div>{kioskMode && !fullscreen && <button type="button" onClick={enterFullscreen} disabled={!supportsFullscreen} className="rounded-full border border-orange-300/30 bg-orange-400/15 px-4 py-2 text-xs font-black text-orange-200 disabled:cursor-default disabled:opacity-80">{supportsFullscreen ? "تفعيل Kiosk / ملء الشاشة" : "Kiosk يعمل عبر متصفح الجهاز"}</button>}</div></header>
      <section key={`${slide?.id ?? "empty"}-${index}`} className="nfood-display-copy-overlay absolute z-15 left-[7vw] bottom-[17vh] max-w-[52vw] text-right">{slide?.badgeText && <span className="mb-3 block text-sm font-black text-orange-100 drop-shadow-[0_3px_8px_rgba(0,0,0,.95)]">{slide.badgeText}</span>}<h1 className="max-w-full break-words text-4xl font-black leading-tight tracking-tight text-white drop-shadow-[0_4px_10px_rgba(0,0,0,.95)] sm:text-5xl md:text-7xl">{displayTitle}</h1>{displaySubtitle && <p className="mt-3 max-w-full break-words text-lg font-bold leading-8 text-white drop-shadow-[0_3px_8px_rgba(0,0,0,.95)] sm:text-xl md:text-3xl md:leading-[1.5]">{displaySubtitle}</p>}{displayTitleEn && <div className="mt-4 max-w-full border-t border-white/25 pt-3 text-left" dir="ltr"><h2 className="max-w-full break-words text-xl font-black leading-tight text-orange-100 drop-shadow-[0_3px_8px_rgba(0,0,0,.95)] sm:text-2xl md:text-4xl">{displayTitleEn}</h2>{displaySubtitleEn && <p className="mt-1 max-w-full break-words text-sm font-semibold leading-7 text-slate-100 drop-shadow-[0_3px_8px_rgba(0,0,0,.95)] sm:text-base md:text-xl">{displaySubtitleEn}</p>}</div>}{match?.callToAction && <span className="mt-5 inline-flex max-w-full break-words rounded-full bg-rose-500/85 px-5 py-2.5 text-base font-black text-white shadow-xl">{match.callToAction}</span>}</section>
      <div key={`ad-${slide?.id ?? "empty"}-${index}`} className="nfood-display-ad-bar absolute inset-x-4 bottom-16 z-20 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-orange-200/25 px-4 py-3 text-right shadow-2xl backdrop-blur-md sm:inset-x-8 md:inset-x-14" style={{ backgroundColor: activePlayback?.screen.adBackground ?? "#111c2ee6" }}><div><p className="text-[9px] font-black uppercase tracking-[0.2em] text-orange-300">NASSER CAFE · {timedOffer.label}</p><p className="mt-1 text-sm font-black text-white sm:text-base">{slide?.badgeText || timedOffer.label}</p><p className="mt-1 text-[10px] font-bold text-slate-300">{displayClock}</p></div><p className="max-w-[60%] text-xs font-bold leading-5 text-orange-50 sm:text-sm">{displayTitle || timedOffer.message} <span className="text-orange-300">· {timedOffer.message}</span></p></div>
    </div>
  </main>;
}

// Legacy regression hook retained as documentation only; the rendered copy is intentionally frameless.
// nfood-display-copy-panel
// nfood-display-slide-enter
