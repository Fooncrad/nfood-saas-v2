...
export default function PublicDisplay() {
  ...
  return <main dir={locale === "ar" ? "rtl" : "ltr"} className="relative min-h-screen overflow-hidden bg-[#07111f] text-white">
    ...
    <div className="relative flex min-h-screen flex-col justify-between p-8 md:p-14">
      <header className="flex items-center justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.28em] text-orange-300">NFOOD DISPLAY</p><p className="mt-2 text-sm font-bold text-slate-300">{playback.data?.screen.name}</p></div><div className="flex items-center gap-2">{match && <div className="rounded-full bg-rose-500 px-4 py-2 text-xs font-black text-white">● وضع المباراة</div>}{matchSeconds !== null && <div className="rounded-full border border-amber-300/40 bg-amber-300/15 px-4 py-2 text-xs font-black text-amber-200">ينتهي خلال {Math.floor(matchSeconds / 60)}:{String(matchSeconds % 60).padStart(2, "0")}</div>}<div className="flex items-center gap-2 rounded-full border border-white/10 bg-white/10 px-4 py-2 text-xs font-bold text-emerald-200"><Wifi className="h-4 w-4" /> مباشر</div>{kioskMode && !fullscreen && <button type="button" onClick={enterFullscreen} className="rounded-full border border-orange-300/30 bg-orange-400/15 px-4 py-2 text-xs font-black text-orange-200">تفعيل Kiosk / ملء الشاشة</button>}</div></header>
      <section className="max-w-4xl"><div className={`mb-6 h-1.5 w-20 rounded-full ${match ? "bg-rose-400" : "bg-orange-400"}`} /><h1 className="text-5xl font-black leading-[1.08] tracking-tight drop-shadow-2xl md:text-8xl">{displayTitle}</h1>{displaySubtitle && <p className="mt-6 max-w-2xl text-xl leading-9 text-slate-200 md:text-3xl">{displaySubtitle}</p>}{match?.callToAction && <span className="mt-8 inline-flex rounded-2xl bg-rose-500 px-6 py-3 text-lg font-black text-white shadow-xl">{match.callToAction}</span>}</section>
      <footer className="flex items-end justify-between gap-4 text-xs font-bold text-slate-400"><span>{match ? "عرض خاطف مباشر" : `${index + 1} / ${slides.length}`}</span><span>{match ? "سيعود للعرض الطبيعي بعد الإيقاف" : kioskMode ? "وضع Kiosk · تشغيل تلقائي بعد فتح الرابط" : "تحديث تلقائي · NFOOD"}</span></footer>
    </div>
  </main>;
}
