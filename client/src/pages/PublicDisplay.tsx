...
export default function PublicDisplay() {
  ...
  const playbackToken = playback.data?.screen.publicToken ?? "";
  ...
  const match = playback.data?.match;
  ...
  const displayImage = match?.mediaFile?.publicUrl ?? slide?.mediaFile?.publicUrl;
  ...
  if (playback.isLoading) return <main className="grid min-h-screen min-h-[100dvh] place-items-center bg-[#07111f] text-white"><div className="text-center"><MonitorPlay className="mx-auto h-12 w-12 animate-pulse text-orange-400" /><p className="mt-4 text-sm font-bold text-slate-300">جاري تحميل شاشة المطعم…</p></div></main>;
  if (playback.isError) return <main dir="rtl" className="grid min-h-screen min-h-[100dvh] place-items-center bg-[#07111f] p-6 text-white"><div className="max-w-md text-center"><WifiOff className="mx-auto h-12 w-12 text-red-400" /><h1 className="mt-5 text-2xl font-black">تعذر تشغيل الشاشة</h1><p className="mt-3 text-sm leading-7 text-slate-400">الرابط غير صالح أو تم إيقاف الشاشة مؤقتًا. اطلب من مدير المطعم إنشاء رابط جديد أو تفعيل الشاشة.</p><p className="mt-4 font-mono text-xs text-slate-600">Request ID: display-{token}</p></div></main>;
  if (!slides.length && !match) return <main dir="rtl" className="grid min-h-screen min-h-[100dvh] place-items-center bg-[#07111f] p-6 text-white"><div className="text-center"><MonitorPlay className="mx-auto h-12 w-12 text-orange-400" /><h1 className="mt-5 text-3xl font-black">{playback.data?.screen.name}</h1><p className="mt-3 text-sm text-slate-400">لا توجد شرائح مؤهلة للعرض حاليًا. ستتم المزامنة تلقائيًا.</p><div className="mt-5 inline-flex items-center gap-2 rounded-full bg-emerald-400/10 px-4 py-2 text-xs font-bold text-emerald-300"><Wifi className="h-4 w-4" /> متصل بالمزامنة</div></div></main>;
  ...
  return <main dir={locale === "ar" ? "rtl" : "ltr"} className="relative min-h-screen min-h-[100dvh] select-none overflow-hidden bg-[#07111f] text-white [touch-action:manipulation]">
    ...
    {displayImage ? <><div className="absolute inset-0 bg-[radial-gradient(circle_at_25%_20%,#e76f3c55,transparent_40%),radial-gradient(circle_at_80%_75%,#2dd4bf33,transparent_35%)]" /><img src={displayImage} alt={displayTitle} className="absolute inset-0 h-full w-full object-contain opacity-95" /></> : <div className="absolute inset-0 bg-[radial-gradient(circle_at_25%_20%,#e76f3c55,transparent_40%),radial-gradient(circle_at_80%_75%,#2dd4bf33,transparent_35%)]" />}
