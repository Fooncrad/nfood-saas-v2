...
export default function PublicDisplay() {
  ...
  const displayImage = match?.mediaFile?.publicUrl ?? slide?.mediaFile?.publicUrl ?? slide?.menuItem?.imageUrl ?? "";
  ...
  const qrPositionClass = ({ "top-left": "left-8 top-8 md:left-14 md:top-14", "top-right": "right-8 top-8 md:right-14 md:top-14", "bottom-left": "bottom-8 left-8 md:bottom-14 md:left-14", "bottom-right": "bottom-8 right-8 md:bottom-14 md:right-14", center: "left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2" } as Record<string, string>)[activePlayback?.screen.qrPosition ?? "bottom-right"];
  ...
  useEffect(() => { slides.forEach((item) => { const url = item?.mediaFile?.publicUrl ?? item?.menuItem?.imageUrl; if (url && typeof window !== "undefined") { const image = new Image(); image.decoding = "async"; image.src = url; } }); }, [slides]);
  ...
  return <main dir={locale === "ar" ? "rtl" : "ltr"} className="relative min-h-screen min-h-[100dvh] select-none overflow-hidden bg-[#07111f] text-white [touch-action:manipulation]">
    ...
    {displayImage ? <><div className="absolute inset-0 scale-105 bg-cover bg-center opacity-60 blur-2xl" style={{ backgroundImage: `url(${displayImage})` }} /><div className="absolute inset-0 bg-[radial-gradient(circle_at_25%_20%,#e76f3c55,transparent_40%),radial-gradient(circle_at_80%_75%,#2dd4bf33,transparent_35%)]" /><img key={`${slide?.id ?? index}-image`} src={displayImage} alt={displayTitle} className="nfood-display-media-enter absolute inset-0 h-full w-full object-contain p-8 opacity-95 drop-shadow-2xl sm:p-12" /></> : <div className="absolute inset-0 bg-[radial-gradient(circle_at_25%_20%,#e76f3c55,transparent_40%),radial-gradient(circle_at_80%_75%,#2dd4bf33,transparent_35%)]"><div className="absolute inset-0 grid place-items-center"><div className="grid h-64 w-64 place-items-center rounded-full border border-orange-300/30 bg-orange-400/10 shadow-[0_0_120px_#e76f3c55]"><div className="text-center"><div className="text-7xl">☕</div><p className="mt-4 text-sm font-black tracking-[0.3em] text-orange-200">NASSER CAFE</p><p className="mt-2 text-xs text-slate-300">نكهة تستحق التوقف</p></div></div></div></div>}
    <div className="absolute inset-0 bg-gradient-to-br from-[#07111f]/90 via-[#07111f]/55 to-[#07111f]/95" />
    {playback.data?.screen.qrEnabled && displayQrUrl && <div className={`absolute z-10 rounded-2xl p-3 shadow-2xl ring-1 ring-white/20 ${qrPositionClass}`} style={{ backgroundColor: playback.data.screen.qrBackground }}><QRCodeSVG value={displayQrUrl} size={playback.data.screen.qrSize} level="H" includeMargin fgColor={playback.data.screen.qrForeground} bgColor={playback.data.screen.qrBackground} /><p className="mt-2 text-center text-[10px] font-black text-white/80">{match ? "امسح للاستفادة من عرض المباراة" : "امسح لفتح المنيو"}</p></div>}
    <div className="relative flex min-h-screen min-h-[100dvh] flex-col justify-between p-4 sm:p-8 md:p-14">
      <header className="flex flex-wrap items-center justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.28em] text-orange-300">NFOOD DISPLAY</p><p className="mt-2 text-sm font-bold text-slate-100">{displayName}</p></div><div className="flex flex-wrap items-center justify-end gap-2">{match && <div className="rounded-full bg-rose-500 px-4 py-2 text-xs font-black text-white">● وضع المباراة</div>}{matchSeconds !== null && <div className="rounded-full border border-amber-300/40 bg-amber-300/15 px-4 py-2 text-xs font-black text-amber-200">ينتهي خلال {Math.floor(matchSeconds / 60)}:{String(matchSeconds % 60).padStart(2, "0")}</div>}<div className={`flex items-center gap-2 rounded-full border px-4 py-2 text-xs font-bold ${connectionState === "connected" ? "border-emerald-300/20 bg-emerald-400/15 text-emerald-200" : connectionState === "connecting" ? "border-amber-300/20 bg-amber-300/15 text-amber-200" : "border-rose-300/20 bg-rose-400/15 text-rose-200"}`}>{connectionState === "connected" ? <Wifi className="h-4 w-4" /> : <WifiOff className="h-4 w-4" />} {connectionState === "connected" ? "متصل" : connectionState === "connecting" ? "جارٍ إعادة الاتصال" : "غير متصل · المحاولة مستمرة"}</div>{kioskMode && !fullscreen && <button type="button" onClick={enterFullscreen} disabled={!supportsFullscreen} className="rounded-full border border-orange-300/30 bg-orange-400/15 px-4 py-2 text-xs font-black text-orange-200 disabled:cursor-default disabled:opacity-80">{supportsFullscreen ? "تفعيل Kiosk / ملء الشاشة" : "Kiosk يعمل عبر متصفح الجهاز"}</button>}</div></header>
      <section key={`${slide?.id ?? "empty"}-${index}`} className="nfood-display-slide-enter max-w-4xl"><div className={`mb-6 h-1.5 w-20 rounded-full ${match ? "bg-rose-400" : "bg-orange-400"}`} /><h1 className="text-4xl font-black leading-[1.08] tracking-tight drop-shadow-2xl sm:text-5xl md:text-8xl">{displayTitle}</h1>{displaySubtitle && <p className="mt-6 max-w-2xl text-xl leading-9 text-slate-200 md:text-3xl">{displaySubtitle}</p>}{match?.callToAction && <span className="mt-8 inline-flex rounded-2xl bg-rose-500 px-6 py-3 text-lg font-black text-white shadow-xl">{match.callToAction}</span>}</section>
      <footer className="flex flex-wrap items-end justify-between gap-3 text-[10px] font-bold text-slate-400 sm:text-xs"><span>{match ? "عرض خاطف مباشر" : `${index + 1} / ${slides.length}`}</span><span>{match ? "سيعود للعرض الطبيعي بعد الإيقاف" : kioskMode ? "وضع Kiosk · تشغيل تلقائي بعد فتح الرابط" : "تحديث تلقائي · NFOOD"}</span></footer>
    </div>
  </main>;
}
