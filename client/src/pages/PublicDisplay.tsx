...
export default function PublicDisplay() {
  ...
  return <main dir={locale === "ar" ? "rtl" : "ltr"} className="relative min-h-screen overflow-hidden bg-[#07111f] text-white">
    ...
    <div className="relative flex min-h-screen flex-col justify-between p-8 md:p-14">
      ...
      <footer className="flex items-end justify-between gap-4 text-xs font-bold text-slate-400"><span>{index + 1} / {slides.length}</span><span>{kioskMode ? "وضع Kiosk · تشغيل تلقائي بعد فتح الرابط" : "تحديث تلقائي · NFOOD"}</span></footer>
    </div>
  </main>;
}
