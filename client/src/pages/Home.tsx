...
const HomePanelFallback = () => <div className="flex min-h-28 items-center justify-center rounded-2xl border border-slate-200/70 bg-white/60 text-sm text-slate-500 shadow-sm backdrop-blur">جارٍ تحميل القسم…</div>;
import { CreateRestaurantDialog } from "@/components/CreateRestaurantDialog";
import { LoyaltyPanel } from "@/components/LoyaltyPanel";
import { ReviewsPanel } from "@/components/ReviewsPanel";
const LazyDriverDeliveryView = lazy(() => import("@/components/DriverDeliveryView").then(({ DriverDeliveryView }) => ({ default: DriverDeliveryView })));
...
const LazyCustomerProfileSettings = lazy(() => import("@/pages/CustomerProfileSettings"));
const LazyCustomerPortal = lazy(() => import("@/pages/CustomerPortal"));
const LazyVcardAccountBinding = lazy(() => import("@/pages/VcardAccountBinding"));
import { DeliveryOperationsPanel } from "@/components/DeliveryOperationsPanel";
const LazyAccountPreferencesPanel = lazy(() => import("@/components/AccountPreferencesPanel").then(({ AccountPreferencesPanel }) => ({ default: AccountPreferencesPanel })));
...
export default function Home() {
  ...
  if (((user.testRole as string | undefined) ?? (user.role === "admin" ? "admin" : "customer")) === "customer") return <Suspense fallback={<HomePanelFallback />}><LazyCustomerPortal /></Suspense>;
  if (isDriver) return <div dir={direction} className="h-dvh min-h-0 overflow-hidden bg-[#f6f7f9] text-slate-900"><aside className={`fixed inset-y-0 z-20 hidden w-64 flex-col bg-[#091321] p-4 text-white shadow-2xl lg:flex ${direction === "rtl" ? "end-0 border-l" : "start-0 border-r"} border-white/10`}><div className="border-b border-white/10 pb-4"><p className="text-lg font-black tracking-[.18em]">NFOOD</p><p className="mt-1 text-xs text-slate-400">لوحة السائق</p></div><div className="mt-5 rounded-2xl bg-orange-500/15 px-3 py-3 text-sm font-bold text-orange-100"><Truck className="mb-2 h-5 w-5" />طلبات التوصيل</div><p className="mt-auto text-[11px] leading-5 text-slate-500">بيانات المطعم والخريطة والإعدادات غير متاحة لهذا الحساب.</p></aside><main className={`h-dvh overflow-y-auto p-4 sm:p-6 ${direction === "rtl" ? "lg:mr-64" : "lg:ml-64"}`}><Suspense fallback={<HomePanelFallback />}><LazyDriverDeliveryView restaurantId={(user as typeof user & { restaurantId?: number } | null)?.restaurantId ?? selectedRestaurantId} /></Suspense></main></div>;
