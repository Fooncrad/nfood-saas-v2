...
export default function Home() {
  ...
  const orders = useMemo(() => (remoteOrders.data ?? []).map((order) => ({ id: `#${order.id}`, table: order.tableName ?? "بدون طاولة", items: order.items?.length ? order.items.map((item) => `${item.quantity} × ${item.itemName}`).join("، ") : "لا توجد بنود محفوظة", total: Number(order.total), status: order.status === "cancelled" ? "completed" : order.status, time: formatOrderTime(order.createdAt), guestName: order.guestName ?? null, guestPhone: order.guestPhone ?? null, customerNote: order.notes ?? null, cashierNotes: order.cashierNotes ?? null, deliveryNote: order.deliveryNote ?? null, channel: order.channel === "dine_in" ? "داخل المطعم" : order.channel === "takeaway" ? "استلام" : order.channel === "reservation" ? "حجز + طلب" : order.channel === "hotel" ? "فندق" : "توصيل", ageMinutes: orderAgeMinutes(order.createdAt), kitchenSectionId: order.kitchenSectionId ?? null, reservationDate: order.reservationDate ?? null, reservationEventType: order.reservationEventType ?? null, partySize: order.partySize ?? null, childrenCount: order.childrenCount ?? null, splitBillMode: order.splitBillMode ?? null })), [remoteOrders.data]);
