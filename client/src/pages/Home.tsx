    ...
    <div dir={direction} lang={language} className="h-dvh min-h-0 overflow-hidden bg-[#f6f7f9] nfood-dashboard-shell text-[#182230] transition-colors duration-300 dark:bg-slate-950 dark:text-slate-100">
      ...
      <main data-testid="dashboard-center-canvas" className={`${direction === "rtl" ? (sidebarCollapsed ? "lg:mr-[72px]" : "lg:mr-[264px]") : (sidebarCollapsed ? "lg:ml-[72px]" : "lg:ml-[264px]")} h-dvh min-h-0 overflow-hidden flex flex-col gap-2 transition-[margin] duration-300 ease-in-out dark:bg-slate-950`}>{isCentralAdmin && pendingTransferCount > 0 && !transferBannerDismissed && <PendingTransferBanner count={pendingTransferCount} language={language as "ar" | "en" | "fr" | "ur"} onOpen={() => setActive("admin")} onClose={() => setTransferBannerDismissed(true)} />}{user?.loginMethod === "admin_impersonation" && <button type="button" disabled={adminReturn.isPending} onClick={() => adminReturn.mutate()} className="fixed bottom-5 left-5 z-50 rounded-2xl bg-[#101d31] px-4 py-3 text-sm font-bold text-white shadow-xl hover:bg-[#17263d]">{adminReturn.isPending ? t("loading") : t("dashboard")}</button>}
        ...
        <div className="nfood-dashboard-content nfood-scroll-area min-h-0 flex-1 overflow-y-auto p-2 pb-4 md:p-3 xl:p-4">
          ...
          {showPreferences ? <AccountPreferencesPanel onClose={() => setShowPreferences(false)} /> : showCustomerProfile ? <CustomerProfileSettings embedded onClose={() => setShowCustomerProfile(false)} /> : showVcardBinding ? <VcardAccountBinding role={user?.testRole === "driver" ? "driver" : "restaurant"} onClose={() => setShowVcardBinding(false)} /> : globalForbiddenAction ? <AccessDeniedView feature={globalForbiddenAction} /> : active === "overview" ? (isCentralAdmin ? <PlatformOverview onNavigate={() => setActive("admin")} /> : <OverviewAnalyticsPanel restaurantId={selectedRestaurantId} orders={visibleOrders} summary={roleSummaryQuery.data} summaryLoading={roleSummaryQuery.isLoading} summaryError={roleSummaryQuery.isError} onNavigate={setActive} />) : !visibleNavItems.some((item) => item.key === active) ? <AccessDeniedView feature={active} /> : <Suspense fallback={<div className="flex min-h-28 items-center justify-center rounded-2xl border border-slate-200/70 bg-white/60 text-sm text-slate-500 shadow-sm backdrop-blur"><span className="mr-2 inline-block h-4 w-4 animate-spin rounded-full border-2 border-[#e76f3c]/25 border-t-[#e76f3c]" />{t("loading")}</div>}><LazyModuleView active={active} orders={visibleOrders} advanceOrder={advanceOrder} orderUpdatePending={updateOrderStatus.isPending} setActive={setActive} restaurantId={selectedRestaurantId} branchId={activeBranchId} ordersLoading={remoteOrders.isLoading} ordersError={remoteOrders.isError} role={user?.testRole} /></Suspense>}
          </div>
        </div>
