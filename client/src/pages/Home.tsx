...
export default function Home() {
  ...
  if (isDriver) return <div dir="rtl" className="h-dvh min-h-0 overflow-hidden bg-[#f6f7f9] text-slate-900"><aside className="fixed inset-y-0 end-0 z-20 hidden w-64 flex-col bg-[#091321] p-4 text-white shadow-2xl lg:flex"><div className="border-b border-white/10 pb-4"><p className="text-lg font-black tracking-[.18em]">NFOOD</p><p className="mt-1 text-xs text-slate-400">لوحة السائق</p></div><div className="mt-5 rounded-2xl bg-orange-500/15 px-3 py-3 text-sm font-bold text-orange-100"><Truck className="mb-2 h-5 w-5" />طلبات التوصيل</div><p className="mt-auto text-[11px] leading-5 text-slate-500">بيانات المطعم والخريطة والإعدادات غير متاحة لهذا الحساب.</p></aside><main className="h-dvh overflow-y-auto p-4 lg:mr-64 sm:p-6"><DriverDeliveryView restaurantId={selectedRestaurantId} /></main></div>;
