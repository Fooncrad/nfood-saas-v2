...
describe("public menu UX", () => {
  it("uses a manager-controlled responsive product grid with consistent lazy-loaded media", () => {
    ...
    expect(page).toContain("nfood-menu-shell ${menuItemLayout === \"grid\" ? \"nfood-menu-grid\" : menuItemLayout === \"cardless\" ? \"nfood-menu-cardless\" : \"nfood-menu-cards\"} flex flex-col");
    expect(page).toContain('menuItemLayout === "grid" ? "nfood-grid-category-groups" : ""');
    expect(styles).toContain(".nfood-menu-shell.nfood-menu-grid .nfood-menu-grid.category-results");
