...
function readStoredLanguage(): Language { if (typeof window === "undefined") return "ar"; const unified = window.localStorage.getItem(LANGUAGE_STORAGE_KEY); if (isUiLanguage(unified)) return unified; const legacyManual = window.localStorage.getItem(MENU_LANGUAGE_MANUAL_STORAGE_KEY); const legacyMenu = window.localStorage.getItem(MENU_LANGUAGE_STORAGE_KEY); const legacyDashboard = window.localStorage.getItem(DASHBOARD_LANGUAGE_STORAGE_KEY); const migrated = [legacyManual, legacyMenu, legacyDashboard].find(isUiLanguage); if (migrated) { window.localStorage.setItem(LANGUAGE_STORAGE_KEY, migrated); window.localStorage.removeItem(MENU_LANGUAGE_STORAGE_KEY); window.localStorage.removeItem(MENU_LANGUAGE_MANUAL_STORAGE_KEY); window.localStorage.removeItem(DASHBOARD_LANGUAGE_STORAGE_KEY); return migrated; } const detected = detectVisitorLanguage(); window.localStorage.setItem(LANGUAGE_STORAGE_KEY, detected); return detected; }
