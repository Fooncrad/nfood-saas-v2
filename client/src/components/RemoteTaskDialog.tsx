  ...
  return <Dialog open={open} onOpenChange={(next) => { if (!next) onClose(); }}>
    <DialogContent dir="rtl" className="max-h-[92vh] overflow-y-auto rounded-3xl border-slate-200 bg-white p-5 shadow-2xl sm:max-w-2xl sm:p-7">
      ...
      <div className="mt-3 grid gap-4 sm:grid-cols-2">
        ...
        <label className="text-xs font-bold">قيمة المهمة (ر.س)<Input value={draft.amount} onChange={(event) => set("amount", event.target.value)} inputMode="decimal" className="mt-2 h-11 rounded-xl bg-slate-50" /></label>
