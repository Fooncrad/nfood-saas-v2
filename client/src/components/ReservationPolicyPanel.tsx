...
export function ReservationPolicyPanel({ restaurantId }: Props) {
  ...
  const [draft, setDraft] = useState({ reservationEnabled: true, reservationMaxPerDay: null as number | null, reservationDepositEnabled: false, reservationDepositAmount: 0, tipsEnabled: false, tipPercent: 0, serviceFeeEnabled: false, serviceFeePercent: 0, reservationNoShowGraceMinutes: 10 });
  useEffect(() => {
    if (!branding.data) return;
    setDraft({
      reservationEnabled: branding.data.reservationEnabled !== false,
      reservationMaxPerDay: branding.data.reservationMaxPerDay == null ? null : Number(branding.data.reservationMaxPerDay),
      reservationDepositEnabled: Boolean(branding.data.reservationDepositEnabled),
      reservationDepositAmount: Number(branding.data.reservationDepositAmount ?? 0),
  ...
  const save = () => {
    if (!branding.data) return;
    const payload = { ...branding.data, ...draft, restaurantId, menuDisplaySettingsJson: branding.data.menuDisplaySettingsJson ?? undefined, brandLogoUrl: branding.data.brandLogoUrl ?? "", pwaInstallMessage: branding.data.pwaInstallMessage ?? "ثبّت منيو مطعمنا للوصول الأسرع", pwaInstallIconUrl: branding.data.pwaInstallIconUrl ?? "", brandDescription: branding.data.brandDescription ?? "", homepageContent: branding.data.homepageContent ?? "", termsOfService: branding.data.termsOfService ?? "", privacyPolicy: branding.data.privacyPolicy ?? "", refundPolicy: branding.data.refundPolicy ?? "", phone: branding.data.phone ?? "", whatsapp: branding.data.whatsapp ?? "", address: branding.data.address ?? "", tipPercent: Math.max(0, Math.min(100, draft.tipPercent)), serviceFeePercent: Math.max(0, Math.min(100, draft.serviceFeePercent)), reservationNoShowGraceMinutes: Math.max(1, Math.min(120, draft.reservationNoShowGraceMinutes)), reservationMaxPerDay: draft.reservationMaxPerDay === null || draft.reservationMaxPerDay <= 0 ? null : Math.min(10000, Math.floor(draft.reservationMaxPerDay)), reservationDepositEnabled: draft.reservationDepositEnabled, reservationDepositAmount: Math.max(0, Math.min(100000, draft.reservationDepositAmount)) };
    ...
    <CardContent className="space-y-4">
      {branding.isLoading ? <div className="rounded-2xl bg-white p-4 text-sm text-slate-500">جارٍ تحميل إعدادات الحجوزات...</div> : branding.isError ? <div className="rounded-2xl bg-rose-50 p-4 text-sm text-rose-700">تعذر تحميل إعدادات الحجوزات. حاول تحديث الصفحة.</div> : <>
        <div className="grid gap-3 md:grid-cols-2">
          ...
          <label className="rounded-2xl border border-slate-200 bg-white p-3 text-sm font-bold">الحد اليومي للحجوزات (اتركه فارغًا لغير محدود)<Input type="number" min="1" step="1" value={draft.reservationMaxPerDay ?? ""} onChange={(e) => setDraft({ ...draft, reservationMaxPerDay: e.target.value === "" ? null : Number(e.target.value) })} className="mt-2 bg-white" dir="ltr" /><Help>يمنع استقبال حجوزات جديدة بعد الوصول إلى هذا العدد في اليوم المحدد. اتركه فارغًا إذا كان الاستقبال غير محدود.</Help></label>
          <label className="rounded-2xl border border-slate-200 bg-white p-3 text-sm font-bold"><span className="flex items-center gap-3"><input type="checkbox" checked={draft.reservationDepositEnabled} onChange={(e) => setDraft({ ...draft, reservationDepositEnabled: e.target.checked })} className="h-4 w-4 accent-indigo-600" /> تفعيل مبلغ الحجز</span><Input type="number" min="0" step="0.01" value={draft.reservationDepositAmount} onChange={(e) => setDraft({ ...draft, reservationDepositAmount: Number(e.target.value) })} className="mt-2 bg-white" dir="ltr" /><Help>يظهر المبلغ للعميل قبل الإرسال. عند تفعيله يُطلب تسجيل الدخول، وتبقى حالة الدفع «معلقة» حتى ربط بوابة الدفع لاحقًا؛ لا يتم تحصيل أي مبلغ الآن.</Help></label>
