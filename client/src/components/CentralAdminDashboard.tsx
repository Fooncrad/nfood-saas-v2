...
const translations = {
  ar: {
    dir: 'rtl', projectName: 'NFOOD', panelTitle: 'لوحة الإدارة العليا', panelSubtitle: 'إدارة المطاعم والمتاجر والاشتراكات والعمليات من مركز واحد', systemReady: 'حالة النظام غير مفحوصة', searchPlaceholder: 'ابحث عن متجر، مستخدم، إعداد... (Ctrl + K)', adminCenter: 'مركز التحكم', adminBadge: 'بوابة التحكم العليا',
  ...
  en: {
    dir: 'ltr', projectName: 'NFOOD Ecosystem', panelTitle: 'Central & Enterprise Admin', panelSubtitle: 'Instant feature toggles, business sector activation & subscription plan upgrades', systemReady: 'System health not checked', searchPlaceholder: 'Search stores, users, settings... (Ctrl + K)', adminCenter: 'Control Center', adminBadge: 'Master Control Gateway',
  ...
  fr: {
    dir: 'ltr', projectName: 'Écosystème NFOOD', panelTitle: 'Admin Central & Entreprise', panelSubtitle: 'Activation instantanée des fonctionnalités, des secteurs et mise à niveau des forfaits', systemReady: 'Santé système non vérifiée', searchPlaceholder: 'Rechercher magasins, utilisateurs, paramètres... (Ctrl + K)', adminCenter: 'Centre de Contrôle', adminBadge: 'Passerelle de Contrôle Maître',
...
export function CentralAdminDashboard({ onToggleTheme, currentTheme, embedded = false }: { onToggleTheme?: () => void, currentTheme: string, embedded?: boolean }) {
  ...
  const sectorCatalogQuery = trpc.admin.sectorCatalog.useQuery(undefined, { retry: 1 });
  // Keep the nine canonical NFOOD sectors visible even when the live governance
  // endpoint is temporarily unavailable. Live values replace these definitions
  // as soon as the query succeeds; Trend Kitchen remains a separate module.
  const canonicalSectors: SectorGovernance[] = [
    { key: 'restaurant', alias: 'restaurant', labelAr: 'المطاعم والمقاهي', labelEn: 'Restaurants & Cafés', labelFr: 'Restaurants & Cafés', active: true, entityCount: 0, coverUrl: '', source: 'platformEntity' },
    { key: 'vegetables', alias: 'veg', labelAr: 'الخضار والفواكه', labelEn: 'Vegetables & Fruits', labelFr: 'Fruits & Légumes', active: true, entityCount: 0, coverUrl: '', source: 'platformEntity' },
    { key: 'grocery', alias: 'grocery', labelAr: 'البقالات والتموينات', labelEn: 'Grocery & Markets', labelFr: 'Épiceries & Marchés', active: true, entityCount: 0, coverUrl: '', source: 'platformEntity' },
    { key: 'laundry', alias: 'laundry', labelAr: 'المغاسل والخدمات', labelEn: 'Laundry & Care', labelFr: 'Blanchisserie & Services', active: true, entityCount: 0, coverUrl: '', source: 'platformEntity' },
    { key: 'automotive', alias: 'auto', labelAr: 'خدمات السيارات', labelEn: 'Automotive Services', labelFr: 'Services Automobiles', active: true, entityCount: 0, coverUrl: '', source: 'platformEntity' },
    { key: 'beauty_salon', alias: 'barber', labelAr: 'الصالونات ومراكز التجميل', labelEn: 'Beauty & Salons', labelFr: 'Beauté & Salons', active: true, entityCount: 0, coverUrl: '', source: 'platformEntity' },
    { key: 'public_works', alias: 'public', labelAr: 'الأشغال العامة والصيانة', labelEn: 'Public Works & Maintenance', labelFr: 'Travaux & Maintenance', active: true, entityCount: 0, coverUrl: '', source: 'platformEntity' },
    { key: 'fashion', alias: 'fashion', labelAr: 'الموضة والأزياء', labelEn: 'Fashion & Apparel', labelFr: 'Mode & Vêtements', active: true, entityCount: 0, coverUrl: '', source: 'platformEntity' },
    { key: 'sweets', alias: 'sweets', labelAr: 'الحلويات والمخبوزات', labelEn: 'Sweets & Bakery', labelFr: 'Douceurs & Boulangerie', active: true, entityCount: 0, coverUrl: '', source: 'platformEntity' },
  ];
  const liveSectors = (sectorCatalogQuery.data?.sectors ?? []).filter((sector) => sector.source === 'platformEntity');
  const sectors: SectorGovernance[] = liveSectors.length > 0 ? liveSectors : canonicalSectors;
  const sectorsUsingFallback = !sectorCatalogQuery.isLoading && liveSectors.length === 0;
    ...
    return (
      <div className="min-h-full bg-[#08121f] p-1 text-slate-100">
        <ActivitiesSectorsView
          ...
          loading={sectorCatalogQuery.isLoading}
          usingFallback={sectorsUsingFallback}
        />
      </div>
    );
  }
