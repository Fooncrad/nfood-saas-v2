...
export default function LanguageSwitcher({ compact = false, allowedLanguages }: { compact?: boolean; allowedLanguages?: Language[] }) {
  ...
  return (
    <details className="relative z-50">
      ...
      <div className="absolute end-0 top-[calc(100%+0.5rem)] min-w-44 overflow-hidden rounded-2xl border border-slate-200 bg-white p-1.5 text-slate-800 shadow-xl ring-1 ring-black/5" role="menu" aria-label={t("chooseLanguage")}>
        {languages.map((item) => (
