...
export default function LanguageSwitcher({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <label className={`inline-flex items-center gap-2 rounded-xl border border-slate-200/80 bg-white/80 px-2.5 py-2 text-xs font-semibold text-slate-700 shadow-sm backdrop-blur transition hover:border-orange-200 hover:shadow-md ${compact ? "h-9" : "h-10"}`}>
      ...
      <select aria-label={t("chooseLanguage")} value={language} onChange={(event) => setLanguage(event.target.value as Language)} className="cursor-pointer appearance-none bg-transparent outline-none">
        {(Object.keys(languageMeta) as Language[]).map((item) => <option key={item} value={item}>{languageMeta[item].nativeLabel}</option>)}
      </select>
    </label>
  );
}
