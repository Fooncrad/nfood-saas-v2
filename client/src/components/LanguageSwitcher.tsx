...
export default function LanguageSwitcher({ compact = false, allowedLanguages, minimal = false }: { compact?: boolean; allowedLanguages?: Language[]; minimal?: boolean }) {
  ...
  return (
    <div className="flex items-center gap-1.5" aria-busy={isLanguageChanging}>
      ...
      <div className="nfood-language-menu absolute end-0 top-[calc(100%+0.5rem)] max-h-[min(22rem,calc(100dvh-5rem))] min-w-44 max-w-[calc(100vw-1rem)] overflow-y-auto overscroll-contain rounded-2xl border border-slate-200 bg-white p-1.5 text-slate-800 shadow-xl ring-1 ring-black/5" role="menu" aria-label={t("navigation.chooseLanguage")}>
        {languages.map((item) => (
