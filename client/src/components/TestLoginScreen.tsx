import { ArrowLeft, Copy, Utensils } from "lucide-react";
...
export function TestLoginScreen({
  ...
  const copyDemo = async (account: (typeof demoAccounts)[number]) => { const credentials = `${account.email}\n123456`; try { await navigator.clipboard.writeText(credentials); toast.success(language === "fr" ? "Identifiants copiés" : language === "en" ? "Demo credentials copied" : "تم نسخ بيانات الدخول التجريبية"); } catch { toast.error(language === "fr" ? "Copie indisponible" : language === "en" ? "Copy is unavailable" : "تعذر النسخ تلقائيًا"); } };
