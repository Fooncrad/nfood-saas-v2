import { ArrowLeft, Check, Copy, LoaderCircle, Utensils } from "lucide-react";
...
export function TestLoginScreen({
  ...
  return (
    <div dir={direction} className="min-h-screen bg-[#f7f8fb] text-slate-900 transition-colors duration-300 dark:bg-slate-950 dark:text-white motion-safe:animate-[nfood-auth-slide-in_360ms_cubic-bezier(0.23,1,0.32,1)]">
      <div className="mx-auto grid min-h-screen max-w-6xl lg:grid-cols-[0.85fr_1.15fr]">
        ...
        <main className="flex items-center justify-center p-5 sm:p-8">
          ...
          <Card className="w-full max-w-xl rounded-[2rem] border border-white/70 bg-white/60 shadow-[0_24px_80px_rgba(15,23,42,0.12)] backdrop-blur-2xl transition-colors duration-300 dark:border-white/10 dark:bg-slate-900/60">
            <CardContent className="p-6 sm:p-10">
              ...
              <div className="space-y-5"><label className="block text-xs font-bold">{t("email")}<div className="relative mt-2"><Input data-testid="login-email" type="text" autoComplete="username" value={email} onChange={(event) => setEmail(event.target.value)} placeholder={language === "en" ? "admin or name@example.com" : language === "fr" ? "admin ou nom@exemple.com" : "admin أو name@example.com"} className="h-13 rounded-2xl border-slate-200 bg-slate-50/70 px-4" /></div>{email.length > 0 && !email.includes("@") && <p role="alert" className="mt-1.5 text-xs font-semibold text-red-600">{language === "en" ? "Enter a valid email address." : language === "fr" ? "Saisissez une adresse e-mail valide." : "أدخل بريدًا إلكترونيًا صحيحًا."}</p>}</label><label className="flex items-center gap-2 text-xs font-bold"><input type="checkbox" checked={rememberMe} onChange={(event) => setRememberMe(event.target.checked)} className="h-4 w-4 accent-[#e76f3c]" />{language === "en" ? "Remember me" : language === "fr" ? "Se souvenir de moi" : "تذكرني"}</label><label className="block text-xs font-bold">{t("password")}<Input data-testid="login-password" type="password" value={password} onChange={(event) => setPassword(event.target.value)} onKeyDown={handleSubmitKeyDown} placeholder={t("passwordPlaceholder")} className="mt-2 h-13 rounded-2xl border-slate-200 bg-slate-50/70 px-4" /></label>{!password && email.length > 0 && <p role="alert" className="-mt-3 text-xs font-semibold text-red-600">{language === "en" ? "Password is required." : language === "fr" ? "Le mot de passe est requis." : "كلمة المرور مطلوبة."}</p>}{loginError && <p role="alert" className="-mt-3 rounded-xl bg-red-50 px-3 py-2 text-xs font-semibold leading-5 text-red-700">{loginError}</p>}<Button data-testid="login-submit" type="button" onClick={() => { if (!email.trim() || !password) { toast.error(language === "en" ? "Enter your email and password first." : language === "fr" ? "Saisissez d’abord votre e-mail et votre mot de passe." : "أدخل البريد الإلكتروني وكلمة المرور أولًا."); return; } onSubmit(); }} disabled={pending} className="h-13 w-full rounded-2xl bg-[#e76f3c] text-base font-bold shadow-lg shadow-orange-200 transition hover:-translate-y-0.5 hover:bg-[#d85f2e] active:scale-[0.98]">{pending ? <><LoaderCircle className="mr-2 h-5 w-5 animate-spin" />{t("signingIn")}</> : t("secureSignIn")}</Button><button type="button" onClick={() => setForgotOpen((current) => !current)} className="w-full text-center text-xs font-bold text-[#c75325] hover:underline">{forgotOpen ? (language === "en" ? "Hide password recovery" : language === "fr" ? "Masquer la récupération" : "إخفاء استعادة كلمة المرور") : (language === "en" ? "Forgot password?" : language === "fr" ? "Mot de passe oublié ?" : "نسيت كلمة المرور؟")}</button>{forgotOpen && <div className="rounded-2xl border border-orange-200 bg-orange-50/70 p-4"><p className="text-xs leading-5 text-orange-900">{language === "en" ? "Enter the account email above and request a recovery link. We will not reveal whether the email is registered." : language === "fr" ? "Saisissez l’e-mail du compte et demandez un lien de récupération. Nous ne révélerons pas si l’adresse est enregistrée." : "أدخل بريد الحساب في الحقل أعلاه ثم اضغط إرسال رابط الاستعادة. لن نكشف ما إذا كان البريد مسجلًا."}</p><Button type="button" onClick={onForgotPassword} disabled={forgotPending || !email.includes("@")} className="mt-3 h-10 w-full rounded-xl bg-[#c75325] text-xs font-bold hover:bg-[#a84420]">{forgotPending ? (language === "en" ? "Sending link..." : language === "fr" ? "Envoi du lien..." : "جارٍ إرسال الرابط...") : (language === "en" ? "Send recovery link" : language === "fr" ? "Envoyer le lien" : "إرسال رابط الاستعادة")}</Button>{forgotMessage && <p className="mt-3 rounded-xl bg-emerald-100 px-3 py-2 text-center text-xs font-bold text-emerald-800">{forgotMessage}</p>}</div>}<div className="relative py-1 text-center"><span className="relative z-10 bg-white px-3 text-[11px] text-slate-400">{t("or")}</span><div className="absolute inset-x-0 top-1/2 h-px bg-slate-100" /></div><Button type="button" variant="outline" onClick={onOAuth} className="h-12 w-full rounded-2xl border-slate-200">{t("continueWithGoogle")}</Button><div className="grid grid-cols-2 gap-2"><Button type="button" disabled className="h-11 rounded-2xl border border-slate-200 bg-slate-50 text-xs text-slate-500">OTP · Demo</Button><Button type="button" disabled className="h-11 rounded-2xl border border-slate-200 bg-slate-50 text-xs text-slate-500">Passkey · Demo</Button></div><p className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-center text-[11px] leading-5 text-amber-800">{t("demoIntegrations")} {t("demoIntegrationsNote")}</p><Button type="button" variant="ghost" onClick={onRegister} className="h-11 w-full rounded-2xl text-[#e76f3c]">{t("joinRestaurant")} <ArrowLeft className="mr-1 h-4 w-4" /></Button></div>
