...
export default function AdminBusinessOnboarding() {
  ...
  const sectors=trpc.marketplace.adminSectors.useQuery(undefined,{retry:1});
  const [open,setOpen]=useState(false);
  const [draft,setDraft]=useState({customerName:"",email:"",countryCode:"SA",sector:"restaurant",plan:"Basic" as typeof PLANS[number],taxId:"",city:"",status:true});
  ...
  return <section dir="rtl" className="space-y-5">
    ...
    <Dialog open={open} onOpenChange={setOpen}><DialogContent dir="rtl" className="max-h-[calc(100dvh-1rem)] overflow-y-auto rounded-3xl sm:max-w-2xl">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <label className="text-sm font-semibold">النشاط<select className="mt-2 h-11 w-full rounded-xl border border-slate-200 bg-white px-3" value={draft.sector} onChange={e=>setDraft({...draft,sector:e.target.value as typeof draft.sector})}>{(sectors.data ?? []).filter(x=>x.isActive).map(x=><option key={x.slug} value={x.slug}>{x.labelAr}</option>)}</select></label>
      ...
      <Button onClick={submit} disabled={createStore.isPending||localization.isLoading||sectors.isLoading||!country||!draft.sector||draft.customerName.trim().length<2||!draft.email.includes("@")} className="w-full rounded-xl bg-[#e76f3c] hover:bg-[#d85f2e]">{createStore.isPending?"جارٍ الإنشاء...":<><CheckCircle2 className="ml-2 h-4 w-4"/>إنشاء وربط المنشأة</>}</Button>
