...
export function DashboardQuickAccess({ items, onNavigate, title, description, storageScope = "default" }: DashboardQuickAccessProps) {
  ...
  const resetOrder = () => { setOrderedKeys([]); try { localStorage.removeItem(`nfood.dashboard.widget-order:${storageScope}`); } catch { /* storage unavailable */ } toast.success(copy.reset); };
