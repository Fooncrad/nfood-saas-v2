...
export function WaiterCallsPanel({ restaurantId, branchId }: { restaurantId: number; branchId?: number }) {
  ...
  const latestCallId = useRef<number | null>(null);
  useEffect(() => {
    const newest = calls.data?.[0];
    if (!newest) return;
    if (latestCallId.current === null) { latestCallId.current = newest.id; return; }
    if (newest.id === latestCallId.current) return;
    latestCallId.current = newest.id;
    toast.info(`نداء جديد من طاولة ${newest.tableName}: ${newest.reason}`);
    try {
      ...
      oscillator.connect(gain); gain.connect(audio.destination); oscillator.start(); oscillator.stop(audio.currentTime + 0.8);
    } catch { /* Browser may require prior user interaction for audio. */ }
  }, [calls.data]);
