...
import { Bell, ChevronDown, LifeBuoy, ReceiptText, Search, Settings2, ShieldCheck, Utensils, Zap } from "lucide-react";
...
export function HomeSidebar({
  ...
  useEffect(() => {
    try { window.localStorage.setItem(sidebarCollapsedKey, String(sidebarCollapsed)); } catch { /* storage may be unavailable */ }
  ...
  return (
    <aside data-sidebar-collapsed={sidebarCollapsed ? "true" : "false"} className={`nfood-unified-sidebar fixed inset-y-0 z-20 hidden h-full w-[304px] overflow-hidden overscroll-contain border-slate-200 bg-[#111c2e] text-white lg:flex lg:flex-col ${direction === "rtl" ? "right-0 border-l" : "left-0 border-r"}`}>
      <div className="flex h-[68px] items-center gap-3 border-b border-white/10 px-5"><button type="button" onClick={() => setSidebarCollapsed((value) => !value)} aria-label={sidebarCollapsed ? "توسيع القائمة الجانبية" : "تصغير القائمة الجانبية"} title={sidebarCollapsed ? "توسيع القائمة الجانبية" : "تصغير القائمة الجانبية"} className="order-first flex h-9 w-9 shrink-0 items-center justify-center rounded-lg text-slate-300 transition hover:bg-white/10 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-300"><ChevronDown className={`h-4 w-4 transition-transform ${sidebarCollapsed ? "-rotate-90" : "rotate-90"}`} /></button>
