  ...
  return (
    <aside className={`fixed inset-y-0 z-20 hidden w-[272px] border-slate-200 bg-[#111c2e] text-white lg:flex lg:flex-col ${direction === "rtl" ? "right-0 border-l" : "left-0 border-r"}`}>
      ...
      <div className="space-y-5 px-4 pt-6">
        ...
        <nav className="space-y-3">{(isCentralAdmin ? platformGroups : sidebarGroups).map((group) => { const isCollapsed = collapsedGroups[group.id ?? group.label] ?? false; return <div key={group.id ?? group.label} className={`rounded-2xl border ${isCentralAdmin ? "border-white/10 bg-white/[.035] p-2" : "border-transparent"}`}><button type="button" onClick={() => setCollapsedGroups((current) => ({ ...current, [group.id ?? group.label]: !isCollapsed }))} className="flex w-full items-center justify-between px-3 py-2 text-right"><span className="text-[10px] font-bold tracking-[.14em] text-slate-500">{group.label}</span><ChevronDown className={`h-3.5 w-3.5 text-slate-500 transition-transform duration-200 ${isCollapsed ? "-rotate-90" : ""}`} /></button>{!isCollapsed && <div className="space-y-1">{group.items.map((item) => { const Icon = item.icon; const isActive = item.key === active; return <button key={item.key} onClick={() => onNavigate(item.key)} className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-right text-[13px] transition-all duration-200 ${isActive ? "bg-[#e76f3c] font-semibold text-white shadow-lg shadow-orange-950/20" : "text-slate-400 hover:bg-white/5 hover:text-white"}`}><Icon className="h-[18px] w-[18px]" /><span>{item.label}</span>{item.key === "orders" && <span className="mr-auto rounded-full bg-white/15 px-2 py-0.5 text-[10px] text-white">{ordersLoading ? "…" : orderCount}</span>}</button>; })}</div>}</div>; })}</nav>
      </div>
