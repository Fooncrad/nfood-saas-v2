...
export function HomeSidebar({
  ...
  return (
    <aside className={`fixed inset-y-0 z-20 hidden h-full w-[304px] overflow-y-auto overscroll-contain border-slate-200 bg-[#111c2e] text-white lg:flex lg:flex-col ${direction === "rtl" ? "right-0 border-l" : "left-0 border-r"}`}>
      ...
      <div className="space-y-3 px-3 pb-3 pt-3">
        <div className="rounded-2xl border border-white/10 bg-white/[.04] p-2.5">
          ...
          <div className="space-y-1.5">
            <select aria-label={t("selectRestaurant")} value={selectedRestaurantId} onChange={(event) => onRestaurantChange(Number(event.target.value))} className="h-9 w-full rounded-xl border border-white/10 bg-[#17263d] px-3 text-xs text-white outline-none"><option value={selectedRestaurantId}>{restaurants.length ? t("selectRestaurant") : restaurantMessage}</option>{restaurants.map((restaurant) => <option key={restaurant.id} value={restaurant.id}>{restaurant.name}</option>)}</select>
            <select aria-label={t("selectBranch")} value={branch} onChange={(event) => onBranchChange(event.target.value)} disabled={(branchesLoading && !branches.length) || branchesError || branches.length === 0} className="h-9 w-full rounded-xl border border-white/10 bg-[#17263d] px-3 text-xs text-white outline-none"><option value="">{branches.length ? t("selectBranch") : branchMessage}</option>{branches.map((item) => <option key={item.id} value={item.name}>{item.name}</option>)}</select>
            <button type="button" onClick={onOpenCommand} className="flex h-9 w-full items-center gap-2 rounded-xl border border-white/10 bg-[#17263d] px-3 text-xs text-slate-300 hover:text-white"><Search className="h-4 w-4" />{t("search")} <span className="mr-auto text-[10px] text-slate-500">Ctrl K</span></button>
          </div>
        </div>
