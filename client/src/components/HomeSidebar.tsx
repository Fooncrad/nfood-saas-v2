  ...
  return (
    <TooltipProvider delayDuration={120}>
      <motion.aside
        ...
        className={`nfood-unified-sidebar fixed inset-y-0 z-20 hidden h-full overflow-hidden overscroll-contain border-white/10 bg-[#091321]/95 text-white shadow-[0_24px_80px_rgba(3,10,20,.42)] backdrop-blur-2xl lg:flex lg:flex-col ${direction === "rtl" ? "end-0 border-l" : "start-0 border-r"}`}
      >
            ...
            {!sidebarCollapsed && (
              <motion.div key="brand" initial={{ opacity: 0, x: direction === "rtl" ? 8 : -8 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: direction === "rtl" ? 8 : -8 }} transition={{ duration: 0.18 }} className="flex min-w-0 flex-1 items-center gap-2.5">
                ...
                <div className="min-w-0">
                  ...
                  <div className="truncate text-[8px] font-semibold tracking-[.14em] text-slate-500">{isCentralAdmin ? "CENTRAL ADMIN" : "RESTAURANT OS"}</div>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
