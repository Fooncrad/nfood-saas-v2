  ...
  return <Card className="rounded-2xl border-amber-100 bg-gradient-to-br from-white to-amber-50/40 shadow-sm">
    ...
    <CardContent className="grid gap-4 lg:grid-cols-[1fr_260px]">
      <div className="space-y-3">
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-3"><div className="mb-2 flex flex-wrap items-center justify-between gap-2"><div><p className="text-xs font-black text-slate-700">القالب الافتراضي للمنيو</p><p className="mt-1 text-[10px] leading-5 text-slate-400">يُطبق تلقائيًا على زوار رابط المنيو. يمكن للزائر تبديله مؤقتًا من المنيو.</p></div><Badge variant="outline" className="rounded-full text-[10px]">{menuTemplate === "glass" ? "NFOOD Glass" : menuTemplate === "bistro" ? "Bistro" : menuTemplate === "market" ? "Market" : menuTemplate === "signature" ? "Signature" : "Editorial"}</Badge></div><div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-5">{menuTemplates.map((item) => <button key={item.key} type="button" onClick={() => setMenuTemplate(item.key)} aria-pressed={menuTemplate === item.key} className={`group rounded-xl border p-2 text-right transition ${menuTemplate === item.key ? "border-orange-400 bg-orange-50 shadow-sm" : "border-slate-200 bg-slate-50 hover:border-orange-200"}`}><span className="mb-2 block h-9 rounded-lg shadow-inner" style={{ background: item.swatch }} /><span className="block text-[11px] font-black text-slate-800">{item.label}</span><span className="mt-0.5 block text-[10px] text-slate-500">{item.description}</span></button>)}</div></div>
