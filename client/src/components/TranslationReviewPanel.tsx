...
import { AlertTriangle, CheckCircle2, Download, Eye, FileJson, Languages, RefreshCw, Search, Sparkles, Upload, X } from "lucide-react";
...
type TranslationEntity = { id: number; type: "category" | "item"; name: string; description?: string | null; translations: TranslationEntry[] };
...
export function TranslationReviewPanel({ restaurantId }: { restaurantId: number }) {
  ...
  const importJson = async (file: File) => {
    ...
    try {
      ...
      for (const entry of updates) {
        ...
        else await updateItem.mutateAsync({ restaurantId, id: entry.id, translationsJson });
      }
      await utils.platform.menuCategories.invalidate({ restaurantId }); await utils.platform.menuItems.invalidate({ restaurantId });
  ...
  return <Card className="mb-6 overflow-hidden rounded-[28px] border-0 bg-[#151922] text-white shadow-[0_20px_60px_rgba(15,23,42,0.18)]" dir="rtl">
    ...
    <CardContent className="space-y-5 p-5"><div className="grid gap-3 md:grid-cols-3">{summary.map((entry) => <div key={entry.code} className="rounded-2xl border border-white/10 bg-white/[0.06] p-4"><div className="flex items-center justify-between"><div><p className="text-lg font-black">{entry.code.toUpperCase()}</p><p className="text-xs text-slate-400">{languages.find((lang) => lang.code === entry.code)?.label}</p></div><span className="text-2xl font-black text-[#71e6c0]">{entry.percent}%</span></div><div className="mt-4 h-2 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-[#12c99a] transition-all" style={{ width: `${entry.percent}%` }} /></div><p className="mt-2 text-[11px] text-slate-400">{entry.complete} من {entry.total} عناصر مكتملة</p></div>)}</div>
      ...
      <div className="space-y-2">{filteredEntities.length === 0 ? <p className="rounded-2xl border border-white/10 bg-white/[0.04] px-4 py-10 text-center text-sm text-slate-400">لا توجد نصوص تطابق البحث.</p> : filteredEntities.slice(0, 40).map((entity) => { const entry = entity.translations.find((item) => item.language === selectedLanguage); const approved = entity.translations.filter((item) => item.status === "approved" || !item.status).length; const hasDraft = entity.translations.some((item) => item.status === "draft"); return <div key={`${entity.type}-${entity.id}`} className="flex flex-col gap-3 rounded-2xl border border-white/10 bg-white/[0.04] p-4 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><p className="truncate font-black">{entry?.name || entity.name}</p><p className="mt-1 truncate text-xs text-slate-400">{entity.type === "category" ? "فئة" : "صنف"} · {approved}/{languages.length} لغات معتمدة · {entry?.description || "لا يوجد وصف لهذه اللغة"}</p></div><div className="flex flex-wrap gap-2"><Button type="button" variant="outline" size="sm" onClick={() => setPreview(entity)} className="rounded-lg border-white/20 bg-transparent text-white hover:bg-white/10"><Eye className="ml-1 h-3.5 w-3.5" /> معاينة</Button>{(hasDraft || !entry) && <Button type="button" size="sm" onClick={() => retryOne(entity, selectedLanguage)} disabled={retryTranslation.isPending} className="rounded-lg bg-[#12c99a] text-[#09251f] hover:bg-[#65edc9]"><RefreshCw className="ml-1 h-3.5 w-3.5" /> إعادة الترجمة</Button>}</div></div>; })}</div>{filteredEntities.length > 40 && <p className="text-center text-xs text-slate-400">تظهر أول 40 نتيجة؛ ضيّق البحث للوصول إلى نص محدد.</p>}
      {preview && <div className="rounded-2xl border border-[#71e6c0]/30 bg-[#71e6c0]/10 p-4"><div className="flex items-center justify-between gap-3"><div><p className="text-xs text-[#71e6c0]">معاينة قبل النشر · {preview.type === "category" ? "فئة" : "صنف"}</p><h3 className="mt-1 font-black">{selectedEntry?.name || preview.name}</h3></div><Button type="button" variant="ghost" onClick={() => setPreview(null)} className="text-white hover:bg-white/10"><X className="ml-1 h-4 w-4" />إغلاق</Button></div><div className="mt-4 rounded-xl bg-black/20 p-4"><div className="flex items-center gap-2 text-xs text-slate-300"><FileJson className="h-4 w-4" /> {selectedLanguage.toUpperCase()}</div><p className="mt-3 font-bold">{selectedEntry?.name || "لا توجد ترجمة"}</p><p className="mt-1 text-xs leading-5 text-slate-300">{selectedEntry?.description || "لا يوجد وصف"}</p></div></div>}</CardContent></Card>;
}
