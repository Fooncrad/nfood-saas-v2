              ...
              {rows.map((store) => (
                <div key={store.id} className="flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-slate-100 bg-slate-50/70 p-4 transition hover:border-orange-200 hover:bg-orange-50/30">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <a href={`/marketplace/manage/${store.id}`} className="inline-flex h-9 items-center gap-1.5 rounded-xl border border-blue-200 bg-blue-50 px-3 text-xs font-bold text-blue-700 transition hover:bg-blue-100" title="الدخول لإدارة المتجر عند الحاجة"><LogIn className="h-4 w-4" />إدارة المتجر</a>
