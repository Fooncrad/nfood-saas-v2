...
export function KdsOperationsBoard({ restaurantId, orders, advanceOrder, orderUpdatePending }: Props) {
  ...
  useEffect(() => { setSectionFilter("all"); }, [restaurantId]);
  const visibleOrders = useMemo(() => filterAndSortKdsOrders(sectionFilter === "all" ? orders : orders.filter((order) => order.kitchenSectionId === sectionFilter), filter, sort), [filter, orders, sectionFilter, sort]);
