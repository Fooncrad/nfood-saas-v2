...
export function CompactOrdersBoard({ orders, advanceOrder, orderUpdatePending, ordersLoading, ordersError, restaurantId, mode = "orders" }: CompactOrdersBoardProps) {
  ...
  const orderAction = (order: Order) => {
    ...
    return next ? <Button type="button" disabled={orderUpdatePending} onClick={() => advanceOrder(order.id)} className="h-8 rounded-lg bg-[#e76f3c] px-3 text-[11px] hover:bg-[#d85f2e]">{orderUpdatePending ? "جارٍ..." : `نقل إلى ${statusLabels[next]}`}</Button> : <span className="inline-flex items-center gap-1 text-[11px] font-bold text-emerald-600"><Eye className="h-3.5 w-3.5" /> مكتمل</span>;
  };

  const orderCard = (order: Order) => <div key={order.id} className="rounded-xl border border-slate-100 bg-slate-50/70 p-3"><div className="flex items-start justify-between gap-2"><div><p className="text-xs font-black">#{order.id}</p><p className="mt-1 text-[11px] text-slate-500">{order.table} · {order.channel}</p></div><Badge variant="outline" className={`rounded-lg px-2 py-1 text-[10px] ${statusStyles[order.status]}`}>{statusLabels[order.status]}</Badge></div><p className="mt-2 truncate text-[11px] text-slate-500">{order.items}</p><div className="mt-2 flex items-center justify-between gap-2 text-[10px] text-slate-400"><span className="flex items-center gap-1"><Clock3 className="h-3 w-3" />منذ {order.ageMinutes} د</span><strong className="text-slate-700">{order.total.toLocaleString("ar-SA")} ر.س</strong></div><div className="mt-2">{orderAction(order)}</div></div>;
