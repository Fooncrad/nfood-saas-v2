...
export function CompactOrdersBoard({ orders, advanceOrder, orderUpdatePending, ordersLoading, ordersError, restaurantId, mode = "orders" }: CompactOrdersBoardProps) {
  ...
  const orderCard = (order: Order) => { const statusTone = getOrderStatusPalette(order.status); const deferred = isDeferredOrder(order); const reservation = reservationSummary(order); const split = splitBillLabel(order.splitBillMode); return <div key={order.id} className={`rounded-xl border p-3 ${deferred ? "border-amber-200 bg-amber-50/60" : "border-slate-100 bg-slate-50/70"}`}><div className="flex items-start justify-between gap-2"><div><p className="text-xs font-black">#{order.id}</p><p className="mt-1 text-[11px] text-slate-500">{order.table} · {order.channel}</p></div><div className="flex flex-wrap justify-end gap-1"><Badge variant="outline" className={`rounded-lg px-2 py-1 text-[10px] ${statusTone.className}`}><span className={`ml-1 inline-block h-1.5 w-1.5 rounded-full ${statusTone.dotClassName}`} />{statusTone.label}</Badge>{deferred && <Badge className="rounded-lg bg-amber-500 px-2 py-1 text-[10px] text-white">مؤجل</Badge>}</div></div><p className="mt-2 truncate text-[11px] text-slate-500">{order.items}</p>{reservation && <p className="mt-2 rounded-lg bg-white/80 px-2 py-1.5 text-[10px] font-bold text-amber-800">موعد الحجز: {reservation}</p>}{split && <p className="mt-1 text-[10px] font-semibold text-violet-700">{split}</p>}<div className="mt-2 flex items-center justify-between gap-2 text-[10px] text-slate-400"><span className="flex items-center gap-1"><Clock3 className="h-3 w-3" />منذ {order.ageMinutes} د</span><strong className="text-slate-700">{order.total.toLocaleString("ar-SA")} ر.س</strong></div><div className="mt-2">{orderAction(order)}</div></div>; };
