...
function reservationSummary(order: Order) { const date = getReservationDate(order.reservationDate); if (!date) return null; const details = [`${date.toLocaleDateString("ar-SA", { day: "numeric", month: "short" })} · ${date.toLocaleTimeString("ar-SA", { hour: "2-digit", minute: "2-digit" })}`]; if (order.partySize) details.push(`${order.partySize} ضيف`); if (order.childrenCount) details.push(`${order.childrenCount} طفل`); return details.join(" · "); }
