...
export function DriverDeliveryView({ restaurantId }: { restaurantId: number }) {
  ...
  return <div dir="rtl" className="space-y-3">
    ...
    <div className="grid gap-3 lg:grid-cols-[minmax(0,1.25fr)_300px]">
      <div className="space-y-2">{deliveryOrders.map((order) => <button key={order.id} type="button" onClick={() => { setSelectedId(order.id); setEta(String(order.deliveryEtaMinutes ?? 20)); }} className={`w-full rounded-2xl border p-3 text-right transition ${selectedId === order.id ? "border-sky-400 bg-sky-50/60 shadow-sm" : "border-slate-200 bg-white hover:border-sky-200"}`}><div className="flex items-start justify-between gap-3"><div><p className="font-black">طلب #{order.id}</p><p className="mt-1 text-xs text-slate-500">بيانات العميل محجوبة للسائق · تظهر تعليمات الاستلام بعد فتح الطلب</p></div><Badge variant="outline" className={`rounded-lg text-[10px] ${getDeliveryStatusPalette(order.deliveryStatus).className}`}><span className="ml-1 inline-block h-1.5 w-1.5 rounded-full bg-current" />{getDeliveryStatusPalette(order.deliveryStatus).label}</Badge></div><div className="mt-3 flex flex-wrap gap-3 text-xs text-slate-500"><span className="flex items-center gap-1"><Clock3 className="h-3.5 w-3.5" /> ETA: {order.deliveryEtaMinutes ? `${order.deliveryEtaMinutes} دقيقة` : "غير محدد"}</span><span>الإجمالي: {order.total} SAR</span></div></button>)}</div>
      <Card className="rounded-2xl border-slate-200"><CardHeader className="px-4 pb-2 pt-4"><CardTitle className="text-base">إدارة التسليم</CardTitle></CardHeader><CardContent className="space-y-2.5 p-4 pt-2">{!selected ? <p className="py-6 text-center text-sm text-slate-500">اختر طلبًا لإدارة حالته.</p> : <><div className="space-y-1 rounded-xl bg-slate-50 p-3 text-xs text-slate-600"><p>الطلب #{selected.id} · بيانات العميل محجوبة</p>{hasPickedUp && <><p>الجوال: {selected.guestPhone || "غير محدد"}</p><p>العنوان: {selected.deliveryAddress || "غير محدد"}</p></>}<p>الدفع: {selected.paymentMethod === "cash" ? "كاش — يسجل على السائق عند الاستلام" : selected.paymentMethod || "غير محدد"}</p>{!canPickup && selected.deliveryStatus === "assigned" && <p className="font-bold text-amber-700">بانتظار تجهيز المطعم وفتح الاستلام</p>}</div><label className="block text-xs font-bold">ETA بالدقائق<Input value={eta} onChange={(event) => setEta(event.target.value)} type="number" min={1} max={1440} className="mt-1.5 h-9 rounded-xl" /></label><label className="block text-xs font-bold">ملاحظة التسليم<Input value={note} onChange={(event) => setNote(event.target.value)} placeholder="مثال: الوصول من البوابة الخلفية" className="mt-1.5 h-9 rounded-xl" /></label>{!isClosed && <SecureDeliveryChat orderId={selected.id} audience="driver" />}<div className="grid grid-cols-2 gap-2">{canPickup && <Button disabled={updateStatus.isPending} onClick={() => update("picked_up")} className={`h-9 rounded-xl text-xs ${actionPalette.operational}`}>استلام الطلب</Button>}{canDepart && <Button disabled={updateStatus.isPending} onClick={() => update("out_for_delivery")} className="h-9 rounded-xl bg-blue-600 text-xs text-white hover:bg-blue-700">في الطريق</Button>}{canComplete && <><Button disabled={updateStatus.isPending} onClick={() => update("delivered")} className={`h-9 rounded-xl text-xs ${actionPalette.success}`}><CheckCircle2 className="ml-1 h-4 w-4" />تم التسليم</Button><Button disabled={updateStatus.isPending} onClick={() => update("returned")} variant="outline" className={`h-9 rounded-xl text-xs ${actionPalette.destructive}`}>مرتجع</Button></>}{!isClosed && !canPickup && !canDepart && !canComplete && <p className="col-span-2 rounded-xl bg-slate-100 p-2 text-center text-xs font-bold text-slate-600">بانتظار تحديث حالة الطلب من المطعم</p>}{isClosed && <p className="col-span-2 rounded-xl bg-emerald-50 p-2 text-center text-xs font-bold text-emerald-700">أُغلقت إجراءات هذا الطلب بعد انتهاء التوصيل</p>}</div><div className="border-t border-slate-100 pt-2.5"><label className="block text-xs font-bold text-red-700">سبب رفض/فشل التوصيل<select value={failureReason} onChange={(event) => setFailureReason(event.target.value as DriverRefusalReason | "")} className="mt-1.5 h-9 w-full rounded-xl border border-red-200 bg-white px-2 text-xs text-slate-700"><option value="">اختر السبب قبل الإرسال</option>{DRIVER_REFUSAL_REASONS.map((reason) => <option key={reason} value={reason}>{reason}</option>)}</select></label><Button disabled={updateStatus.isPending || !failureReason} onClick={() => update("failed")} variant="outline" className={`mt-2 h-9 w-full rounded-xl text-xs ${actionPalette.destructive}`}><AlertTriangle className="ml-1 h-4 w-4" />تسجيل رفض التوصيل وإبلاغ المطعم</Button></div></>}</CardContent></Card>
    </div>
  </div>;
}
