...
export default function FloatingSupportActions() {
  ...
  return <div className="fixed right-4 top-1/2 z-[64] hidden -translate-y-1/2 flex-col gap-2 rounded-2xl border border-white/70 bg-white/85 p-1.5 shadow-xl backdrop-blur-xl sm:flex dark:border-slate-700 dark:bg-slate-900/85" dir="ltr">{links.map(({ label, href, icon: Icon, tone }) => <a key={label} href={href} target="_blank" rel="noreferrer" aria-label={label} title={label} className={`flex h-10 w-10 items-center justify-center rounded-xl bg-slate-100 text-slate-600 transition hover:-translate-x-1 hover:text-white active:scale-95 motion-safe:hover:animate-pulse dark:bg-slate-800 dark:text-slate-200 ${tone}`}><Icon className="h-4 w-4" /></a>)}</div>;
}
