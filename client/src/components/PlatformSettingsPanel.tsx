...
type SettingKey = "supportEmail" | "supportPhone" | "defaultCurrency" | "defaultTimezone" | "maintenanceMode" | "allowGuestCheckout";
...
export function PlatformSettingsPanel() {
  ...
  const [draft, setDraft] = useState<Settings>({ supportEmail: "", supportPhone: "", defaultCurrency: "SAR", defaultTimezone: "Asia/Riyadh", maintenanceMode: "false", allowGuestCheckout: "true" });
  useEffect(() => { if (settingsQuery.data) setDraft(settingsQuery.data); }, [settingsQuery.data]);
  const save = (key: SettingKey) => updateSetting.mutate({ key, value: draft[key] });
  return <Card className="mt-5 rounded-2xl border-orange-100 bg-white shadow-sm" dir="rtl">
    <CardHeader><CardTitle className="text-base">إعدادات المنصة المركزية</CardTitle><p className="text-xs text-slate-500">تُدار من Super Admin وتؤثر على الإعدادات الافتراضية دون كشف أسرار المزودات.</p></CardHeader>
