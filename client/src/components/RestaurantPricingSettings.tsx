...
import { Globe2, Percent, Save } from "lucide-react";
...
export function RestaurantPricingSettings({ restaurantId }: { restaurantId: number }) {
  ...
  const update = trpc.platform.updateRestaurantPricing.useMutation({
    onSuccess: () => { toast.success("تم تحديث الدولة والعملة والخصم والضريبة فورياً"); void query.refetch(); },
    onError: (error) => toast.error(`تعذر تحديث إعدادات التسعير: ${error.message}`),
  });
  ...
  useEffect(() => {
    ...
    setDiscount(String(query.data.defaultDiscountPercent ?? "0"));
