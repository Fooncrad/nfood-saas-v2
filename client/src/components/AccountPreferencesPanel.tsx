...
export function AccountPreferencesPanel({ onClose }: { onClose?: () => void }) {
  ...
  const preferences = trpc.platform.myPreferences.useQuery(undefined, {
    retry: false,
  });
  ...
  const save = trpc.platform.saveMyPreferences.useMutation({
    ...
    onError: error => toast.error(`تعذر حفظ التفضيلات: ${error.message}`),
  });
  ...
  return (
    <Card className="rounded-3xl border-0 bg-white shadow-xl" dir="rtl">
      <CardHeader>
        <CardTitle className="text-xl font-black">تفضيلات الحساب</CardTitle>
      </CardHeader>
      <CardContent className="space-y-5">
        <div>
          <p className="mb-2 text-sm font-bold">لغة لوحة التحكم</p>
            ...
            {languageOptions.map(([value, label]) => (
              <button
                ...
                className={`rounded-xl border px-2 py-2 text-xs font-bold ${
                  language === value
                    ? "border-[#e76f3c] bg-orange-50 text-[#d85f2e]"
        ...
        <div>
          <p className="mb-2 text-sm font-bold">المظهر</p>
        ...
        <div>
          <p className="mb-2 text-sm font-bold">قالب اللون</p>
          <select
            value={themePreset}
            onChange={event => setThemePreset(event.target.value)}
            className="h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm"
          >
            {themePresetOptions.map(([value, label]) => (
        ...
        <div className="flex gap-2">
          <Button
            onClick={() => save.mutate({ language, themeMode, themePreset })}
            disabled={save.isPending}
            className="rounded-xl bg-[#e76f3c] text-white"
          >
            {save.isPending ? "جارٍ الحفظ..." : "حفظ التفضيلات"}
          </Button>
          {onClose && (
