...
export function SecureDeliveryChat({ orderId, audience = "participant" }: { orderId: number; audience?: "participant" | "driver" | "restaurant" }) {
  ...
  const submit = () => { const value = body.trim(); if (!value || audience === "driver") return; send.mutate({ orderId, body: value }); };
  const visibleMessages = (messages.data ?? []).filter((message) => audience !== "driver" || message.senderRole === "restaurant" || message.senderRole === "admin");
