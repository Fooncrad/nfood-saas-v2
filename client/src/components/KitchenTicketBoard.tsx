...
export function KitchenTicketBoard({ restaurantId, orders, station }: { restaurantId: number; orders: TicketOrder[]; station?: KitchenStation }) {
  ...
  const sections = slaQuery.data ?? [];
  const stationSectionIds = useMemo(() => getStationSectionIds(sections, station), [sections, station]);
  const activeOrders = useMemo(() => orders.filter((order) => order.status !== "completed" && (!station || (order.kitchenSectionId !== null && order.kitchenSectionId !== undefined && stationSectionIds.has(order.kitchenSectionId)))), [orders, station, stationSectionIds]);
  ...
  useEffect(() => { if (!selectedOrder || !activeOrders.some((order) => Number(order.id.replace("#", "")) === selectedOrderId)) setSelectedOrderId(Number(activeOrders[0]?.id.replace("#", "")) || null); }, [activeOrders, selectedOrder, selectedOrderId]);
  ...
  const ticketSections = useMemo(() => ticketQuery.data?.sections.filter((section) => !station || (section.kitchenSectionId !== null && stationSectionIds.has(section.kitchenSectionId))) ?? [], [ticketQuery.data?.sections, station, stationSectionIds]);
  const printTicket = () => {
    if (!ticketSections.length || !selectedOrder) { toast.error("لا توجد تذكرة جاهزة للطباعة"); return; }
    const body = ticketSections.map((section) => `<section><h2>قسم ${section.kitchenSectionId ?? "غير معيّن"}</h2>${section.items.map((item) => `<p>${item.quantity} × ${item.itemName}</p>`).join("")}</section>`).join("");
  ...
  return <Card className="mt-5 rounded-2xl border-slate-200 bg-white shadow-sm" dir="rtl">
    <CardHeader className="flex-row items-center justify-between border-b border-slate-100"><div><CardTitle className="flex items-center gap-2 text-base"><Layers3 className="h-5 w-5 text-[#e76f3c]" /> {station ? getStationLabel(station) : "تذاكر أقسام المطبخ"}</CardTitle><p className="mt-1 text-xs text-slate-500">اختر طلبًا لعرض الأصناف موزعة على القسم أو التصنيف المرتبط به.</p></div><Button variant="outline" size="sm" onClick={printTicket} disabled={!ticketSections.length} className="rounded-xl"><Printer className="ml-1 h-4 w-4" /> طباعة</Button></CardHeader>
    <CardContent className="p-4">
      {activeOrders.length === 0 ? <div className="py-8 text-center text-sm text-slate-500"><ChefHat className="mx-auto mb-2 h-8 w-8 text-slate-300" />لا توجد طلبات مفتوحة موزعة على {station ? getStationLabel(station) : "أقسام المطبخ"}.</div> : <div className="grid gap-4 lg:grid-cols-[220px_1fr]">
