...
type TicketOrder = { id: string; table: string; time: string; status: string; ageMinutes?: number };
...
export function KitchenTicketBoard({ restaurantId, orders }: { restaurantId: number; orders: TicketOrder[] }) {
  ...
  const printTicket = () => {
    ...
    popup.document.write(`<html dir="rtl"><head><title>طلب ${selectedOrder.id}</title><style>body{font-family:Arial;padding:24px}h1{font-size:22px}h2{font-size:16px;border-bottom:1px solid #ddd;padding-bottom:8px}p{font-size:15px}</style></head><body><h1>NFOOD · ${selectedOrder.id}</h1><p>${selectedOrder.table} · ${selectedOrder.time}</p>${body}<script>window.onload=()=>window.print()</script></body></html>`);
  ...
  return <Card className="mt-5 rounded-2xl border-slate-200 bg-white shadow-sm" dir="rtl">
    ...
    <CardContent className="p-4">
      {activeOrders.length === 0 ? <div className="py-8 text-center text-sm text-slate-500"><ChefHat className="mx-auto mb-2 h-8 w-8 text-slate-300" />لا توجد طلبات مفتوحة لتقسيمها على أقسام المطبخ.</div> : <div className="grid gap-4 lg:grid-cols-[220px_1fr]">
        <div className="space-y-2">{activeOrders.map((order) => { const numericId = Number(order.id.replace("#", "")); return <button key={order.id} onClick={() => setSelectedOrderId(numericId)} className={`w-full rounded-xl border p-3 text-right ${selectedOrderId === numericId ? "border-[#e76f3c] bg-orange-50" : "border-slate-100 bg-slate-50/70 hover:border-orange-200"}`}><div className="flex items-center justify-between text-xs font-bold"><span>{order.id}</span><Badge variant="outline" className={`rounded-md text-[10px] ${getOrderStatusPalette(order.status).className}`}><span className="ml-1 inline-block h-1.5 w-1.5 rounded-full bg-current" />{getOrderStatusPalette(order.status).label}</Badge></div><p className="mt-2 text-xs text-slate-500">{order.table} · {order.time}</p><span className={`mt-2 inline-flex rounded-md px-2 py-1 text-[10px] font-bold ${order.ageMinutes !== undefined && order.ageMinutes >= 15 ? "bg-red-50 text-red-700" : "bg-slate-100 text-slate-500"}`}>منذ {order.ageMinutes ?? 0} د</span></button>; })}</div>
