...
export function ContentOrdersPanel({ restaurantId }: { restaurantId: number }) {
  ...
  const [receiptPreview, setReceiptPreview] = useState<{ orderId: number; url: string } | null>(null);
  ...
  const analyzeReceipt = trpc.restaurantContent.analyzeContentReceipt.useMutation({ onSuccess: async () => { toast.success("تم استخراج المبلغ والتاريخ كمقترح للمراجعة"); await utils.restaurantContent.contentPurchaseOrders.invalidate({ restaurantId }); }, onError: (error) => toast.error(`تعذر تحليل الإيصال: ${error.message}`) });
  ...
  return <><Card className="rounded-3xl border-slate-200 bg-white shadow-sm">
    ...
    <CardContent className="space-y-4 p-5">
      ...
      <div className="space-y-3">{filteredOrders.map((order) => { const meta = statusMeta[order.status as ContentOrderStatus] ?? statusMeta.unpaid; const StatusIcon = meta.icon; const items = parseItems(order.itemsJson); return <article key={order.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition hover:border-orange-200 hover:shadow-md"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><h3 className="font-black text-slate-900">طلب محتوى #{order.id}</h3><Badge className={`gap-1 rounded-full border ${meta.className}`}><StatusIcon className="h-3.5 w-3.5" />{meta.label}</Badge></div><p className="mt-1 text-xs text-slate-500">{order.customerName || "عميل بدون اسم"}{order.customerPhone ? ` · ${order.customerPhone}` : ""} · {new Date(order.createdAt).toLocaleString("ar-SA")}</p></div><div className="text-left"><p className="text-lg font-black text-[#e76f3c]">{order.total} {order.currencyCode}</p><p className="text-[11px] text-slate-400">{items.length} عنصر</p></div></div><div className="mt-4 grid gap-2 sm:grid-cols-2">{items.map((item, index) => <div key={`${order.id}-${item.listingId ?? index}`} className="flex items-center justify-between gap-3 rounded-xl bg-slate-50 px-3 py-2 text-xs"><span className="truncate font-bold text-slate-700">{item.title || "محتوى مرئي"}</span><span className="shrink-0 font-black text-slate-500">{item.price} {item.currencyCode || order.currencyCode}</span></div>)}</div>{(order.receiptExtractedAmount || order.receiptExtractedDate) && <div className="mt-3 rounded-xl border border-violet-100 bg-violet-50 px-3 py-2 text-xs text-violet-900"><p className="font-black">اقتراح التحليل الذكي</p><p className="mt-1">المبلغ: <strong>{order.receiptExtractedAmount || "غير واضح"}</strong> · التاريخ: <strong>{order.receiptExtractedDate || "غير واضح"}</strong> · الثقة: {Math.round(Number(order.receiptExtractionConfidence ?? 0) * 100)}%</p></div>}<div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-3"><div className="flex flex-wrap items-center gap-2">{order.receiptUrl ? <><Button type="button" variant="outline" size="sm" onClick={() => setReceiptPreview({ orderId: order.id, url: order.receiptUrl! })} className="rounded-xl"><FileImage className="ml-2 h-4 w-4" />معاينة الإيصال</Button><Button type="button" variant="ghost" size="sm" onClick={() => analyzeReceipt.mutate({ orderId: order.id })} disabled={analyzeReceipt.isPending} className="rounded-xl text-[#e76f3c]">{analyzeReceipt.isPending ? "جارٍ التحليل..." : "قراءة المبلغ والتاريخ"}</Button></> : <span className="text-xs font-bold text-slate-400">لم يرفق العميل إيصالًا بعد</span>}{order.note && <span className="max-w-full truncate text-xs text-slate-500" title={order.note}>ملاحظة: {order.note}</span>}</div><label className="flex items-center gap-2 text-xs font-bold text-slate-600">تحديث الحالة<select value={order.status} disabled={updateStatus.isPending} onChange={(event) => updateStatus.mutate({ restaurantId, id: order.id, status: event.target.value as ContentOrderStatus })} className="h-9 rounded-lg border border-slate-200 bg-white px-2 text-xs font-black outline-none focus:border-orange-400">{Object.entries(statusMeta).map(([value, item]) => <option key={value} value={value}>{item.label}</option>)}</select></label></div></article>; })}</div>
    </CardContent>
  </Card><Dialog open={Boolean(receiptPreview)} onOpenChange={(open) => { if (!open) setReceiptPreview(null); }}><DialogContent className="max-w-2xl rounded-3xl"><DialogHeader><DialogTitle>معاينة إيصال التحويل · طلب #{receiptPreview?.orderId}</DialogTitle></DialogHeader>{receiptPreview?.url && <div className="overflow-hidden rounded-2xl border border-slate-200 bg-slate-50 p-2"><img src={receiptPreview.url} alt={`إيصال طلب المحتوى رقم ${receiptPreview.orderId}`} className="max-h-[70vh] w-full object-contain" /></div>}<p className="text-center text-xs text-slate-500">المعاينة للرجوع والمطابقة فقط، ولا يتم اعتماد الدفع تلقائيًا.</p></DialogContent></Dialog></>;
}
