...
export function ContentOrdersPanel({ restaurantId }: { restaurantId: number }) {
  ...
  return <Card className="rounded-3xl border-slate-200 bg-white shadow-sm">
    ...
    <CardContent className="space-y-4 p-5">
      <div className="grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-4"><p className="text-[11px] font-bold text-slate-500">إجمالي الطلبات</p><p className="mt-1 text-2xl font-black text-slate-900">{summary.total}</p></div><div className="rounded-2xl bg-amber-50 p-4"><p className="text-[11px] font-bold text-amber-700">تحتاج مراجعة</p><p className="mt-1 text-2xl font-black text-amber-900">{summary.verifying}</p></div><div className="rounded-2xl bg-emerald-50 p-4"><p className="text-[11px] font-bold text-emerald-700">مقبولة</p><p className="mt-1 text-2xl font-black text-emerald-900">{summary.approved}</p></div></div>
