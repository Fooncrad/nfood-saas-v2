...
import { Megaphone, MonitorPlay, Plus, Save, Trash2 } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent className="grid gap-5 lg:grid-cols-[230px_1fr]">
          ...
          <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50/70 p-4">
            {!activeScreen ? <div className="flex min-h-40 items-center justify-center text-center text-sm text-slate-500">اختر شاشة أو أنشئ شاشة لإضافة الأطباق والصور.</div> : <>
              <div className="mb-4 flex flex-wrap items-center justify-between gap-2"><div><p className="text-sm font-black">{activeScreen.name}</p><p className="mt-1 text-xs text-slate-500">أضف طبقًا من المنيو أو صورة من المكتبة.</p></div><span className="rounded-full bg-white px-3 py-1 text-[10px] font-mono text-slate-500">{activeScreen.deviceKey}</span></div>
              ...
              <div className="mt-4 grid gap-2 rounded-2xl border border-white bg-white p-3 sm:grid-cols-2"><select value={slideItemId} onChange={(e) => setSlideItemId(e.target.value)} className="h-10 rounded-xl border border-slate-200 px-3 text-xs"><option value="">اختر طبقًا من المنيو</option>{(menu.data ?? []).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><select value={slideMediaId} onChange={(e) => setSlideMediaId(e.target.value)} className="h-10 rounded-xl border border-slate-200 px-3 text-xs"><option value="">اختر صورة من المكتبة</option>{(media.data ?? []).map((file) => <option key={file.id} value={file.id}>{file.originalName}</option>)}</select><Input value={slideTitle} onChange={(e) => setSlideTitle(e.target.value)} placeholder="عنوان العرض (اختياري)" className="rounded-xl" /><Input value={slideSubtitle} onChange={(e) => setSlideSubtitle(e.target.value)} placeholder="وصف قصير أو عرض" className="rounded-xl" /><Button onClick={() => saveSlide.mutate({ restaurantId, screenId: activeScreen.id, menuItemId: slideItemId ? Number(slideItemId) : null, mediaFileId: slideMediaId ? Number(slideMediaId) : null, title: slideTitle.trim() || null, subtitle: slideSubtitle.trim() || null, sortOrder: activeScreen.slides.length, durationSeconds: 8, isActive: true })} disabled={saveSlide.isPending || (!slideItemId && !slideMediaId)} className="gap-2 rounded-xl bg-[#111c2e] text-xs sm:col-span-2"><Save className="h-4 w-4" /> حفظ طبق في الشاشة</Button></div>
            </>}
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="rounded-3xl border-slate-200 shadow-sm">
        ...
        <CardContent><div className="grid gap-3 rounded-2xl bg-orange-50/60 p-4 sm:grid-cols-2 lg:grid-cols-3"><select value={campaignId} onChange={(e) => setCampaignId(e.target.value)} className="h-10 rounded-xl border border-orange-100 bg-white px-3 text-xs"><option value="">اختر الحملة</option>{(campaigns.data ?? []).map((campaign) => <option key={campaign.id} value={campaign.id}>{campaign.name}</option>)}</select><select value={locale} onChange={(e) => setLocale(e.target.value)} className="h-10 rounded-xl border border-orange-100 bg-white px-3 text-xs"><option value="ar">العربية</option><option value="en">English</option><option value="fr">Français</option><option value="ur">اردو</option></select><Input value={headline} onChange={(e) => setHeadline(e.target.value)} placeholder="العنوان التسويقي" className="rounded-xl bg-white" /><textarea value={body} onChange={(e) => setBody(e.target.value)} placeholder="النص التسويقي والوصف" className="min-h-24 rounded-xl border border-orange-100 bg-white p-3 text-sm outline-none focus:ring-2 focus:ring-orange-200" /><Input value={callToAction} onChange={(e) => setCallToAction(e.target.value)} placeholder="زر الإجراء: اطلب الآن" className="rounded-xl bg-white" /><Button onClick={() => saveContent.mutate({ restaurantId, campaignId: Number(campaignId), locale: locale as "ar" | "en" | "fr" | "ur", headline: headline.trim(), body: body.trim() || null, callToAction: callToAction.trim() || null, menuItemId: slideItemId ? Number(slideItemId) : null, mediaFileId: slideMediaId ? Number(slideMediaId) : null, sortOrder: 0, isApproved: false })} disabled={saveContent.isPending || !campaignId || headline.trim().length < 2} className="gap-2 rounded-xl bg-[#e76f3c] text-xs lg:col-span-3"><Save className="h-4 w-4" /> حفظ النص للمراجعة</Button></div><p className="mt-3 text-[11px] leading-5 text-slate-500">النصوص الجديدة تُحفظ كمسودة مراجعة ولا تُنشر تلقائيًا قبل الاعتماد.</p></CardContent>
      </Card>
    </div>
  );
}
