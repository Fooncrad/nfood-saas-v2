  ...
  if (active === "kds")
    return (
        ...
        <>
          ...
          {ordersLoading && (
            <div className="mb-4 rounded-xl bg-slate-100 px-4 py-3 text-sm text-slate-500">
              جارٍ تحميل الطلبات من قاعدة البيانات...
            </div>
          )}
          {ordersError && (
            <div className="mb-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
              تعذر تحميل الطلبات. Request ID: orders-{restaurantId}
            </div>
          )}
          {!ordersLoading && !ordersError && orders.length === 0 && (
            <div className="mb-4 rounded-xl border border-slate-200 bg-slate-50 px-4 py-4 text-sm text-slate-600">
              لا توجد طلبات محفوظة لهذا الفرع بعد.
            </div>
          )}
          <KdsOperationsBoard
            key={restaurantId}
