...
import { defaultMenuDisplaySettings, normalizeMenuDisplaySettings, type MenuDisplayToolKey, type MenuGridColumns, type MenuImageRatio, type MenuItemLayout, type MenuDetailWindowSettings } from "@shared/menuDisplaySettings";
...
export function ModuleView({
  ...
  > = {
    ...
    languages: {
      ...
      description: "إدارة لغات المنيو ومراجعة الترجمات وحماية أسماء العلامة التجارية.",
      icon: Languages,
    },
    security: {
  ...
  if (active === "languages") return <LanguageSettingsPanel restaurantId={restaurantId} />;
  if (active === "security")
...
const restaurantMenuTemplates = [
  ...
  { id: "customer", name: "واجهة العميل", description: "منيو وسلة بطابع ضيافة فاخر", swatch: "linear-gradient(135deg, #fffdfa 0%, #f1e6d8 56%, #8b5a2b 100%)" },
] as const;
...
function BrandingPanel({ restaurantId, section = "all" }: { restaurantId: number; section?: "all" | "identity" | "layouts" }) {
  ...
  const [draft, setDraft] = useState({
    ...
    brandTextColor: "#172033",
    brandFontFamily: "IBM Plex Sans Arabic",
    brandHeadingFontFamily: "IBM Plex Sans Arabic",
    ...
    themePreset: "nfood-sunset" as
      ...
      | "midnight-berry"
      | "ocean-mint",
    menuTemplate: "editorial" as "editorial" | "bistro" | "glass" | "customer",
    ...
    customDomain: "",
    dashboardPrimaryColor: "#e76f3c",
    dashboardAccentColor: "#f59e0b",
  });
  useEffect(() => {
    if (brandingQuery.data)
      setDraft({
        ...
        brandAccentColor: brandingQuery.data.brandAccentColor ?? "#f59e0b",
        brandTextColor: brandingQuery.data.brandTextColor ?? "#172033",
        brandFontFamily: brandingQuery.data.brandFontFamily ?? "IBM Plex Sans Arabic",
        brandHeadingFontFamily: brandingQuery.data.brandHeadingFontFamily ?? "IBM Plex Sans Arabic",
        ...
        themePreset: brandingQuery.data.themePreset as
          ...
          | "midnight-berry"
          | "ocean-mint",
        menuTemplate: brandingQuery.data.menuTemplate as "editorial" | "bistro" | "glass" | "customer",
        ...
        menuTemplateScheduleFallback: (() => { try { const parsed = JSON.parse(brandingQuery.data.menuTemplateScheduleJson ?? "{}"); return ["editorial", "bistro", "glass", "customer"].includes(parsed.fallbackTemplate) ? parsed.fallbackTemplate : brandingQuery.data.menuTemplate as MenuTemplateId; } catch { return brandingQuery.data.menuTemplate as MenuTemplateId; } })(),
        ...
        customPagesJson: brandingQuery.data.customPagesJson ?? "[]",
        ...
        dashboardPrimaryColor: brandingQuery.data.brandColor ?? "#e76f3c",
  ...
  const updateBranding = trpc.platform.updateBranding.useMutation({
    onSuccess: () => {
      ...
      window.localStorage.setItem("nfood-dashboard-theme-active", JSON.stringify({ primary: draft.brandColor, accent: draft.brandAccentColor }));
      void utils.platform.branding.invalidate();
      toast.success("تم حفظ هوية المطعم ولوحة الفريق");
    },
  ...
  const menuLayoutTemplates = trpc.platform.menuLayoutTemplates.useQuery({ restaurantId }, { enabled: Boolean(user), retry: false });
  const [layoutTemplateName, setLayoutTemplateName] = useState("");
  const createLayoutTemplate = trpc.platform.createMenuLayoutTemplate.useMutation({
    onSuccess: async () => {
      setLayoutTemplateName("");
      await menuLayoutTemplates.refetch();
      toast.success("تم حفظ تخطيط المنيو كقالب جاهز");
    },
    onError: error => toast.error(`تعذر حفظ القالب: ${error.message}`),
  });
  const deleteLayoutTemplate = trpc.platform.deleteMenuLayoutTemplate.useMutation({
    onSuccess: async () => {
      await menuLayoutTemplates.refetch();
      toast.success("تم حذف القالب المحفوظ");
    },
    onError: error => toast.error(`تعذر حذف القالب: ${error.message}`),
  });
  const layoutTemplateSnapshot = useMemo(() => JSON.stringify({ menuTemplate: draft.menuTemplate, glassGlowColor: draft.glassGlowColor, glassCardOpacity: draft.glassCardOpacity, menuDisplaySettingsJson: draft.menuDisplaySettingsJson }), [draft.glassCardOpacity, draft.glassGlowColor, draft.menuDisplaySettingsJson, draft.menuTemplate]);
  const saveLayoutTemplate = () => {
    const name = layoutTemplateName.trim();
    if (name.length < 2) { toast.error("اكتب اسمًا واضحًا للقالب"); return; }
    createLayoutTemplate.mutate({ restaurantId, name, settingsJson: layoutTemplateSnapshot });
  };
  const applyLayoutTemplate = (settingsJson: string) => {
    try {
      const parsed = JSON.parse(settingsJson) as { menuTemplate?: unknown; glassGlowColor?: unknown; glassCardOpacity?: unknown; menuDisplaySettingsJson?: unknown };
      setDraft(current => ({
        ...current,
        menuTemplate: parsed.menuTemplate === "editorial" || parsed.menuTemplate === "bistro" || parsed.menuTemplate === "glass" ? parsed.menuTemplate : current.menuTemplate,
        glassGlowColor: typeof parsed.glassGlowColor === "string" ? parsed.glassGlowColor : current.glassGlowColor,
        glassCardOpacity: typeof parsed.glassCardOpacity === "number" ? parsed.glassCardOpacity : current.glassCardOpacity,
        menuDisplaySettingsJson: typeof parsed.menuDisplaySettingsJson === "string" ? JSON.stringify(normalizeMenuDisplaySettings(parsed.menuDisplaySettingsJson)) : current.menuDisplaySettingsJson,
      }));
      toast.success("تم تطبيق القالب على المعاينة والمسودة");
    } catch { toast.error("تعذر قراءة القالب المحفوظ"); }
  };
  const publicPreviewUrl = useMemo(() => {
    ...
    url.searchParams.set("brandColor", draft.brandColor);
    url.searchParams.set("brandAccentColor", draft.brandAccentColor);
    url.searchParams.set("brandTextColor", draft.brandTextColor);
    url.searchParams.set("brandFont", draft.brandFontFamily);
    url.searchParams.set("brandHeadingFont", draft.brandHeadingFontFamily);
    return url.toString();
  }, [brandingQuery.data?.slug, draft.brandAccentColor, draft.brandColor, draft.brandFontFamily, draft.brandHeadingFontFamily, draft.brandTextColor, draft.glassCardOpacity, draft.glassGlowColor, draft.menuTemplate, publicOrigin]);
  ...
  const setMenuItemLayout = (itemLayout: MenuItemLayout) => setDraft(current => ({ ...current, menuDisplaySettingsJson: JSON.stringify({ ...menuDisplayDraft, itemLayout }) }));
  ...
  const setItemsPerCategory = (itemsPerCategory: number) => setDraft(current => ({ ...current, menuDisplaySettingsJson: JSON.stringify({ ...menuDisplayDraft, itemsPerCategory: Math.max(1, Math.min(200, Math.round(itemsPerCategory))) }) }));
  const setMenuImageRatio = (imageRatio: MenuImageRatio) => setDraft(current => ({ ...current, menuDisplaySettingsJson: JSON.stringify({ ...menuDisplayDraft, imageRatio }) }));
  ...
  const persistMenuDisplaySettings = (nextSettings: typeof defaultMenuDisplaySettings) => {
    ...
    updateBranding.mutate({ restaurantId, ...brandingDraft, menuDisplaySettingsJson: JSON.stringify(nextSettings), menuTemplateScheduleJson: JSON.stringify({ enabled: draft.menuTemplateScheduleEnabled, timezone: draft.menuTemplateScheduleTimezone, fallbackTemplate: draft.menuTemplateScheduleFallback, rules: draft.menuTemplateScheduleRules }), menuTemplateScheduleTimezone: draft.menuTemplateScheduleTimezone });
  };
  const updateDetailWindowSetting = <K extends keyof MenuDetailWindowSettings>(key: K, value: MenuDetailWindowSettings[K]) => {
    const nextSettings = normalizeMenuDisplaySettings(JSON.stringify({ ...menuDisplayDraft, detailWindow: { ...menuDisplayDraft.detailWindow, [key]: value } }));
    setDraft(current => ({ ...current, menuDisplaySettingsJson: JSON.stringify(nextSettings) }));
    persistMenuDisplaySettings(nextSettings);
  };
  ...
  const setMenuColor = (key: "darkBackgroundColor" | "darkSurfaceColor" | "darkTextColor" | "menuBorderColor", value: string) => setDraft(current => ({ ...current, menuDisplaySettingsJson: JSON.stringify({ ...menuDisplayDraft, [key]: value }) }));
    ...
    <div className="space-y-6" data-branding-settings data-branding-section={section}>
        ...
        {brandingQuery.isError ? (
          ...
          <div className="max-w-5xl">
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="grid gap-4 rounded-2xl border border-orange-100 bg-orange-50/50 p-4 sm:grid-cols-2" data-brand-identity-customization>
                <div className="space-y-3">
                  <div>
                    <p className="text-sm font-black text-slate-900">ألوان العلامة التجارية</p>
                    <p className="mt-1 text-xs leading-5 text-slate-500">حدّد لوحة الألوان التي ستظهر في العناوين والأزرار والروابط في تجربة المطعم.</p>
                  </div>
                  <div className="grid gap-3 sm:grid-cols-3">
                    {([
                      { key: "brandColor", label: "اللون الأساسي", fallback: "#e76f3c" },
                      { key: "brandAccentColor", label: "لون التمييز", fallback: "#f59e0b" },
                      { key: "brandTextColor", label: "لون النص", fallback: "#172033" },
                    ] as const).map((color) => (
                      <label key={color.key} className="space-y-2 text-xs font-bold text-slate-700">
                        {color.label}
                        <div className="flex items-center gap-2">
                          <input
                            type="color"
                            value={/^#[0-9A-Fa-f]{6}$/.test(draft[color.key]) ? draft[color.key] : color.fallback}
                            onChange={event => setDraft({ ...draft, [color.key]: event.target.value })}
                            className="h-10 w-11 shrink-0 cursor-pointer rounded-lg border border-slate-200 bg-white p-1"
                            aria-label={color.label}
                          />
                          <Input
                            value={draft[color.key]}
                            onChange={event => setDraft({ ...draft, [color.key]: event.target.value })}
                            className="h-10 rounded-xl px-2 font-mono text-xs"
                            dir="ltr"
                ...
                <div className="space-y-3">
                  <div>
                    <p className="text-sm font-black text-slate-900">خطوط العلامة التجارية</p>
                    <p className="mt-1 text-xs leading-5 text-slate-500">اختر خطًا عربيًا واضحًا للنصوص وخطًا مميزًا للعناوين.</p>
                  </div>
                  <div className="grid gap-3 sm:grid-cols-2">
                    <label className="space-y-2 text-xs font-bold text-slate-700">
                      خط النصوص
                      <select value={draft.brandFontFamily} onChange={event => setDraft({ ...draft, brandFontFamily: event.target.value })} className="h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-xs font-bold text-slate-800 outline-none focus:border-orange-400 focus:ring-2 focus:ring-orange-100">
                        {(["IBM Plex Sans Arabic", "Cairo", "Tajawal", "Noto Sans Arabic", "Almarai"] as const).map(font => <option key={font} value={font}>{font}</option>)}
                      </select>
                    </label>
                    <label className="space-y-2 text-xs font-bold text-slate-700">
                      خط العناوين
                      <select value={draft.brandHeadingFontFamily} onChange={event => setDraft({ ...draft, brandHeadingFontFamily: event.target.value })} className="h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-xs font-bold text-slate-800 outline-none focus:border-orange-400 focus:ring-2 focus:ring-orange-100">
                        {(["IBM Plex Sans Arabic", "Cairo", "Tajawal", "Noto Sans Arabic", "Almarai"] as const).map(font => <option key={font} value={font}>{font}</option>)}
                      </select>
                    </label>
                  </div>
              ...
              <label className="space-y-2 text-sm font-semibold sm:col-span-2">
                صفحات المطعم الاختيارية
                <textarea
                  value={draft.customPagesJson}
                  onChange={event => setDraft({ ...draft, customPagesJson: event.target.value })}
                  className="mt-1 min-h-32 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 font-mono text-xs outline-none"
                  placeholder='[{"slug":"about","title":"عن المطعم","content":"نبذة...","published":true}]'
                  aria-label="الصفحات الاختيارية بصيغة JSON"
                />
                <span className="block text-[11px] font-normal text-slate-500">أضف أي عدد من الصفحات بصيغة JSON: slug وtitle وcontent وpublished. ستظهر المنشورة فقط للعملاء.</span>
              </label>
              <label className="space-y-2 text-sm font-semibold sm:col-span-2">
                الشروط والأحكام
                <textarea
      ...
      <Card id="menu-layouts" className="rounded-2xl border-orange-200 bg-white shadow-sm" data-menu-layouts-card>
        ...
        <CardContent className="space-y-4 p-4 sm:p-6">
                        <section className="space-y-4 rounded-2xl border border-orange-100 bg-orange-50/50 p-4 sm:col-span-2" data-menu-display-settings data-menu-layouts-content><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="mb-2 inline-flex items-center gap-2 rounded-full bg-orange-100 px-3 py-1 text-[10px] font-black text-orange-800"><SlidersHorizontal className="h-3.5 w-3.5" />مركز التحكم المرئي</div><h3 className="text-base font-black text-slate-900">أدوات تخطيط المنيو</h3><p className="mt-1 max-w-2xl text-xs leading-5 text-slate-500">مكان واحد للتحكم في شكل المنيو بالكامل: القوالب، البطاقات، الصور، الألوان، الخطوط، التنقل، وشريط الفئات، مع معاينة مباشرة قبل الحفظ.</p></div><div className="flex items-center gap-2"><div className="flex flex-wrap items-center gap-1.5"><span className="rounded-full bg-white px-2.5 py-1 text-[10px] font-bold text-orange-700">معاينة فورية</span><button type="button" onClick={openPublicPreview} disabled={!publicPreviewUrl} className="rounded-lg bg-slate-900 px-2.5 py-1.5 text-[10px] font-black text-white hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-50">فتح المعاينة العامة</button><button type="button" onClick={resetMenuCustomization} className="rounded-lg border border-orange-200 bg-white px-2.5 py-1.5 text-[10px] font-black text-orange-700 hover:bg-orange-50">إعادة ضبط الافتراضي</button></div></div></div><nav aria-label="أقسام تخطيطات المنيو" className="flex flex-wrap gap-2 rounded-xl border border-orange-100 bg-white/75 p-2"><span className="rounded-lg bg-slate-900 px-3 py-1.5 text-[10px] font-black text-white">القوالب</span><span className="rounded-lg bg-orange-50 px-3 py-1.5 text-[10px] font-bold text-orange-800">البطاقات والصور</span><span className="rounded-lg bg-orange-50 px-3 py-1.5 text-[10px] font-bold text-orange-800">الألوان والخطوط</span><span className="rounded-lg bg-orange-50 px-3 py-1.5 text-[10px] font-bold text-orange-800">التنقل والفئات</span></nav>                        <section className="space-y-4 rounded-2xl border border-slate-200 bg-slate-950 p-4 text-white shadow-sm" data-menu-layout-live-preview>
                          <div className="flex flex-wrap items-start justify-between gap-3">
                            <div>
                              <p className="text-[10px] font-black uppercase tracking-[0.18em] text-orange-300">LIVE PREVIEW · مسودة</p>
                              <h3 className="mt-1 text-base font-black">معاينة تخطيط المنيو قبل الحفظ</h3>
                              <p className="mt-1 max-w-2xl text-xs leading-5 text-white/60">كل تغيير في القالب، الألوان، الخطوط، نسبة الصورة أو عدد الأعمدة يظهر هنا فورًا، ولا يُطبّق على المنيو العام حتى تضغط «حفظ الهوية».</p>
                            </div>
                            <span className="rounded-full border border-white/10 bg-white/10 px-3 py-1 text-[10px] font-black text-orange-200">{draft.menuTemplate === "editorial" ? "تحرير فاخر" : draft.menuTemplate === "bistro" ? "بيسترو دافئ" : draft.menuTemplate === "glass" ? "NFOOD Glass" : "واجهة العميل"}</span>
                          </div>
                          <div className="overflow-hidden rounded-2xl border border-white/10" style={{ backgroundColor: menuDisplayDraft.menuBackgroundColor, color: menuDisplayDraft.cardTextColor, fontFamily: draft.brandFontFamily }}>
                            <div className="flex flex-wrap items-center justify-between gap-3 p-4" style={{ backgroundColor: draft.brandColor, color: "#ffffff", fontFamily: draft.brandHeadingFontFamily }}>
                              <div><p className="text-[10px] font-bold opacity-75">تجربة عميل NFOOD</p><p className="mt-1 text-lg font-black">{draft.brandName || "اسم المطعم"}</p></div>
                              <span className="rounded-full px-3 py-1 text-[10px] font-black" style={{ backgroundColor: draft.brandAccentColor }}>مسودة حية</span>
                            </div>
                            <div className="grid gap-3 p-4 sm:grid-cols-2" style={{ gridTemplateColumns: `repeat(${menuDisplayDraft.gridColumns === 1 ? 1 : 2}, minmax(0, 1fr))` }}>
                              {["طبق اليوم", "برغر ناصر", "سلطة موسمية", "حلوى البيت"].map((itemName, index) => <article key={itemName} className={`flex min-w-0 gap-3 p-3 ${menuDisplayDraft.itemLayout === "cards" ? "rounded-xl border" : "border-b border-dashed"}`} style={{ borderColor: `${draft.brandAccentColor}55`, backgroundColor: draft.brandTextColor === "#172033" ? "#ffffff" : `${draft.brandTextColor}08` }}>
                                <div className="w-20 shrink-0 rounded-lg" style={{ aspectRatio: menuDisplayDraft.imageRatio === "portrait" ? "4 / 5" : menuDisplayDraft.imageRatio === "landscape" ? "4 / 3" : "1 / 1", background: `linear-gradient(135deg, ${draft.brandAccentColor}, ${draft.brandColor})` }} aria-hidden="true" />
                                <div className="flex min-w-0 flex-1 flex-col justify-between"><div><p className={`text-sm font-black ${menuDisplayDraft.oneLineItemName ? "truncate" : "line-clamp-2"}`} style={{ fontFamily: draft.brandHeadingFontFamily, color: draft.brandTextColor }}>{itemName}</p><p className="mt-1 line-clamp-2 text-[10px] opacity-65">شرح مبسط للصنف يظهر هنا قبل فتح التفاصيل.</p></div><div className="mt-2 flex flex-wrap items-center justify-between gap-2"><span className="text-xs font-black" style={{ color: draft.brandColor }}>{[32, 38, 24, 18][index]} ر.س</span><span className="rounded-md px-2 py-1 text-[10px] font-black" style={{ backgroundColor: menuDisplayDraft.cartButtonStyle === "outline" ? "transparent" : draft.brandAccentColor, color: menuDisplayDraft.cartButtonStyle === "outline" ? draft.brandAccentColor : "#ffffff", border: menuDisplayDraft.cartButtonStyle === "outline" ? `1px solid ${draft.brandAccentColor}` : undefined }}>+ السلة</span></div></div>
                              </article>)}
                            </div>
                          </div>
                        </section>
                        <section className="space-y-3 rounded-xl border border-orange-100 bg-white p-3" data-saved-menu-layout-templates>
                          <div className="flex flex-wrap items-start justify-between gap-3">
                            <div><p className="text-xs font-black text-slate-800">قوالب تخطيطات محفوظة</p><p className="mt-1 text-[10px] leading-5 text-slate-500">احفظ إعدادات التخطيط الحالية باسم واضح، ثم بدّل إليها لاحقًا دون إعادة ضبط الخيارات يدويًا.</p></div>
                            <span className="rounded-full bg-orange-50 px-2 py-1 text-[10px] font-bold text-orange-700">{menuLayoutTemplates.data?.length ?? 0} قالب</span>
                          </div>
                          <div className="grid gap-2 sm:grid-cols-[1fr_auto]">
                            <Input value={layoutTemplateName} onChange={event => setLayoutTemplateName(event.target.value)} placeholder="مثال: منيو رمضان · عرض الهاتف" aria-label="اسم قالب تخطيط المنيو" className="h-10 rounded-xl" />
                            <Button type="button" onClick={saveLayoutTemplate} disabled={createLayoutTemplate.isPending} className="h-10 rounded-xl bg-slate-900 px-4 text-xs font-black hover:bg-slate-800">{createLayoutTemplate.isPending ? "جارٍ الحفظ..." : "حفظ التخطيط الحالي كقالب"}</Button>
                          </div>
                          {menuLayoutTemplates.isLoading ? <p className="rounded-xl bg-slate-50 p-4 text-center text-xs text-slate-500">جارٍ تحميل القوالب المحفوظة...</p> : menuLayoutTemplates.isError ? <p className="rounded-xl bg-red-50 p-4 text-xs font-bold text-red-700">تعذر تحميل القوالب. يمكنك متابعة تعديل المسودة وحفظها لاحقًا.</p> : menuLayoutTemplates.data?.length ? <div className="grid gap-2 sm:grid-cols-2">{menuLayoutTemplates.data.map(template => <div key={template.id} className="flex min-w-0 items-center justify-between gap-3 rounded-xl border border-slate-100 bg-slate-50 p-3"><div className="min-w-0"><p className="truncate text-xs font-black text-slate-800">{template.name}</p><p className="mt-1 text-[10px] text-slate-500">آخر تحديث: {new Date(template.updatedAt).toLocaleDateString("ar-SA")}</p></div><div className="flex shrink-0 gap-1"><Button type="button" onClick={() => applyLayoutTemplate(template.settingsJson)} className="h-8 rounded-lg bg-orange-500 px-2.5 text-[10px] font-black hover:bg-orange-600">تطبيق</Button><Button type="button" variant="outline" onClick={() => { if (window.confirm(`حذف قالب «${template.name}»؟`)) deleteLayoutTemplate.mutate({ restaurantId, id: template.id }); }} className="h-8 rounded-lg border-red-100 px-2.5 text-[10px] font-black text-red-600 hover:bg-red-50">حذف</Button></div></div>)}</div> : <p className="rounded-xl border border-dashed border-slate-200 p-4 text-center text-xs text-slate-500">لم تُحفظ قوالب بعد. جرّب تعديل التخطيط ثم احفظ أول قالب لك.</p>}
                        </section>
                        <section className="rounded-xl border border-orange-100 bg-white p-3" data-menu-template-controls><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-xs font-black text-slate-800">القالب الأساسي للمنيو</p><p className="mt-1 text-[10px] text-slate-500">اختر النمط من داخل تخطيطات المنيو، وستظهر النتيجة فورًا في المعاينة.</p></div><span className="rounded-full bg-orange-50 px-2 py-1 text-[10px] font-bold text-orange-700">جزء من التخطيطات</span></div><div className="mt-3 grid gap-2 sm:grid-cols-3">{restaurantMenuTemplates.map(template => <button key={template.id} type="button" aria-pressed={draft.menuTemplate === template.id} onClick={() => setDraft(current => ({ ...current, menuTemplate: template.id }))} className={`rounded-xl border p-2 text-right transition ${draft.menuTemplate === template.id ? "border-orange-400 bg-orange-50 shadow-sm" : "border-slate-200 bg-slate-50 hover:border-orange-200"}`}><span className="mb-2 block h-7 rounded-lg" style={{ background: template.swatch }} /><span className="block text-xs font-black text-slate-800">{template.name}</span><span className="mt-0.5 block text-[10px] text-slate-500">{template.description}</span></button>)}</div></section><section className="rounded-xl border border-orange-200 bg-orange-50/40 p-3" data-menu-item-layout-controls><div className="flex flex-wrap items-start justify-between gap-2"><div><p className="text-xs font-black text-slate-800">نمط عرض أصناف المنيو</p><p className="mt-1 text-[11px] leading-5 text-slate-500">اختر طريقة عرض الأصناف التي يراها عملاء المطعم. يمكنك التبديل بين النسختين في أي وقت ثم حفظ التخطيطات.</p></div><span className="rounded-full bg-white px-2 py-1 text-[10px] font-black text-orange-700">3 أنماط محفوظة</span></div><div className="mt-3 grid gap-2 sm:grid-cols-3"><button type="button" aria-pressed={menuDisplayDraft.itemLayout === "cards"} onClick={() => setMenuItemLayout("cards")} className={`flex items-start gap-3 rounded-xl border p-3 text-start transition ${menuDisplayDraft.itemLayout === "cards" ? "border-orange-500 bg-white shadow-sm" : "border-orange-100 bg-white/60 hover:border-orange-300"}`}><span className="mt-0.5 grid h-9 w-9 shrink-0 grid-cols-2 gap-1 rounded-lg bg-slate-900 p-1.5"><span className="rounded-sm bg-white/90" /><span className="rounded-sm bg-orange-300" /><span className="rounded-sm bg-orange-300" /><span className="rounded-sm bg-white/90" /></span><span><span className="block text-xs font-black text-slate-800">بطاقات</span><span className="mt-1 block text-[10px] leading-4 text-slate-500">حدود وظل خفيفان يفصلان كل صنف بصريًا.</span></span></button><button type="button" aria-pressed={menuDisplayDraft.itemLayout === "grid"} onClick={() => setMenuItemLayout("grid")} className={`flex items-start gap-3 rounded-xl border p-3 text-start transition ${menuDisplayDraft.itemLayout === "grid" ? "border-orange-500 bg-white shadow-sm" : "border-orange-100 bg-white/60 hover:border-orange-300"}`}><span className="mt-0.5 grid h-9 w-9 shrink-0 grid-cols-2 gap-1 rounded-lg bg-orange-500 p-1.5"><span className="rounded-sm bg-white" /><span className="rounded-sm bg-white" /><span className="rounded-sm bg-white" /><span className="rounded-sm bg-white" /></span><span><span className="block text-xs font-black text-slate-800">بطاقات ثنائية</span><span className="mt-1 block text-[10px] leading-4 text-slate-500">صنفان جنبًا إلى جنب مثل الاختيارات المقترحة.</span></span></button><button type="button" aria-pressed={menuDisplayDraft.itemLayout === "cardless"} onClick={() => setMenuItemLayout("cardless")} className={`flex items-start gap-3 rounded-xl border p-3 text-start transition ${menuDisplayDraft.itemLayout === "cardless" ? "border-orange-500 bg-white shadow-sm" : "border-orange-100 bg-white/60 hover:border-orange-300"}`}><span className="mt-0.5 grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-slate-900"><span className="h-px w-5 bg-orange-300" /><span className="mt-1 h-px w-5 bg-white/90" /></span><span><span className="block text-xs font-black text-slate-800">بدون بطاقات</span><span className="mt-1 block text-[10px] leading-4 text-slate-500">تخطيط حر يستفيد من مساحة أكبر مع بقاء المحتوى كاملًا.</span></span></button></div><p className="mt-2 text-[10px] font-bold text-orange-800">النمط الحالي: {menuDisplayDraft.itemLayout === "cards" ? "بطاقات" : menuDisplayDraft.itemLayout === "grid" ? "بطاقات ثنائية" : "بدون بطاقات"}</p></section><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{(Object.keys(menuToolLabels) as MenuDisplayToolKey[]).map(key => <label key={key} className="flex cursor-pointer items-center gap-2 rounded-xl border border-white bg-white px-3 py-2 text-xs font-bold text-slate-700 shadow-sm"><input type="checkbox" checked={menuDisplayDraft.tools[key]} onChange={() => toggleMenuTool(key)} className="h-4 w-4 accent-orange-500" />{menuToolLabels[key]}</label>)}</div><label className="flex cursor-pointer items-center gap-2 rounded-xl border border-white bg-white px-3 py-2 text-xs font-bold text-slate-700 shadow-sm"><input type="checkbox" checked={menuDisplayDraft.showCustomerAccount} onChange={event => setDraft(current => ({ ...current, menuDisplaySettingsJson: JSON.stringify({ ...menuDisplayDraft, showCustomerAccount: event.target.checked }) }))} className="h-4 w-4 accent-orange-500" />إظهار التسجيل والدخول للعميل في الهيدر</label><div className="rounded-xl border border-white bg-white p-3"><p className="text-xs font-black text-slate-700">تخطيط شبكة الأصناف</p><p className="mt-1 text-[11px] text-slate-500">اختر عدد البطاقات في الصف على الشاشات الكبيرة.</p><div className="mt-2 grid grid-cols-4 gap-2">{([4, 3, 2, 1] as MenuGridColumns[]).map(columns => <button key={columns} type="button" onClick={() => setMenuGridColumns(columns)} aria-pressed={menuDisplayDraft.gridColumns === columns} className={`rounded-xl border px-2 py-2 text-xs font-black transition ${menuDisplayDraft.gridColumns === columns ? "border-orange-500 bg-orange-500 text-white shadow-sm" : "border-slate-100 bg-slate-50 text-slate-600 hover:border-orange-200"}`}>{columns}×{columns}<span className="mt-1 block text-[10px] font-medium opacity-70">{columns === 4 ? "كثيف" : columns === 3 ? "متوازن" : columns === 2 ? "واسع" : "مفرد"}</span></button>)}</div><label className="mt-3 block rounded-xl border border-orange-200 bg-white p-3 text-xs font-black text-slate-700">عدد الأصناف الظاهرة في كل قسم<select value={menuDisplayDraft.itemsPerCategory} onChange={event => setItemsPerCategory(Number(event.target.value))} className="mt-2 h-10 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 text-sm"><option value="30">30 صنفًا (الافتراضي)</option><option value="50">50 صنفًا</option><option value="100">100 صنف</option><option value="150">150 صنفًا</option><option value="200">200 صنف</option></select><span className="mt-1 block text-[10px] font-normal leading-4 text-slate-500">يمكن للمطعم زيادة العدد، وسيظهر زر «عرض المزيد» عند تجاوز القيمة المختارة.</span></label><div className="mt-3 rounded-xl border border-orange-200 bg-slate-950 p-3 text-white" data-menu-grid-live-preview><div className="flex items-center justify-between gap-2"><div><p className="text-xs font-black">معاينة مباشرة للتخطيط</p><p className="mt-1 text-[10px] text-white/60">تتغير فورًا قبل اعتماد الحفظ: {menuDisplayDraft.gridColumns}×{menuDisplayDraft.gridColumns}</p></div><span className="rounded-full bg-orange-500/20 px-2 py-1 text-[10px] font-black text-orange-200">مسودة</span></div><div className="mt-3 grid gap-1.5" style={{ gridTemplateColumns: `repeat(${menuDisplayDraft.gridColumns}, minmax(0, 1fr))` }}>{Array.from({ length: menuDisplayDraft.gridColumns === 1 ? 4 : menuDisplayDraft.gridColumns * 2 }, (_, index) => <div key={index} className="min-h-10 rounded-lg border border-white/10 bg-white/10 p-1.5"><div className="h-4 rounded bg-white/15" /><div className="mt-1 h-1.5 w-3/4 rounded bg-orange-300/70" /></div>)}</div></div><div className="mt-3 grid gap-3 lg:grid-cols-[1fr_1fr]"><div className="rounded-xl border border-white bg-white p-3"><p className="text-xs font-black text-slate-700">نسبة صورة الصنف</p><p className="mt-1 text-[11px] text-slate-500">تحكم في شكل الصورة داخل بطاقة المنيو.</p><div className="mt-2 grid grid-cols-3 gap-2">{([{ id: "square", label: "مربعة", ratio: "1 / 1" }, { id: "portrait", label: "طولية", ratio: "4 / 5" }, { id: "landscape", label: "أفقية", ratio: "4 / 3" }] as const).map(option => <button key={option.id} type="button" onClick={() => setMenuImageRatio(option.id)} aria-pressed={menuDisplayDraft.imageRatio === option.id} className={`rounded-lg border px-2 py-2 text-[11px] font-black ${menuDisplayDraft.imageRatio === option.id ? "border-orange-500 bg-orange-500 text-white" : "border-slate-100 bg-slate-50 text-slate-600 hover:border-orange-200"}`}>{option.label}<span className="mt-1 block text-[10px] font-normal opacity-70">{option.ratio}</span></button>)}</div></div><label className="flex cursor-pointer items-start gap-2 rounded-xl border border-white bg-white p-3 text-xs font-bold text-slate-700"><input type="checkbox" checked={menuDisplayDraft.oneLineItemName} onChange={event => setOneLineItemName(event.target.checked)} className="mt-0.5 h-4 w-4 accent-orange-500" /><span><span className="block">اسم الصنف في سطر واحد</span><span className="mt-1 block text-[10px] font-normal text-slate-500">يقلل الزحام البصري ويقص الاسم الطويل بسطر واحد.</span></span></label></div><div className="mt-3 grid gap-3 lg:grid-cols-3"><label className="rounded-xl border border-white bg-white p-3 text-xs font-bold text-slate-700">خلفية المنيو<div className="mt-2 flex items-center gap-2"><input type="color" value={menuDisplayDraft.menuBackgroundColor} onChange={event => setMenuCustomizationValue({ menuBackgroundColor: event.target.value })} className="h-9 w-12 cursor-pointer rounded-lg border border-slate-200 bg-white p-1" aria-label="لون خلفية المنيو" /><Input value={menuDisplayDraft.menuBackgroundColor} onChange={event => setMenuCustomizationValue({ menuBackgroundColor: event.target.value })} dir="ltr" className="h-9 font-mono text-xs" /></div></label><label className="rounded-xl border border-white bg-white p-3 text-xs font-bold text-slate-700">نصوص البطاقات<div className="mt-2 flex items-center gap-2"><input type="color" value={menuDisplayDraft.cardTextColor} onChange={event => setMenuCustomizationValue({ cardTextColor: event.target.value })} className="h-9 w-12 cursor-pointer rounded-lg border border-slate-200 bg-white p-1" aria-label="لون نصوص البطاقات" /><Input value={menuDisplayDraft.cardTextColor} onChange={event => setMenuCustomizationValue({ cardTextColor: event.target.value })} dir="ltr" className="h-9 font-mono text-xs" /></div></label><div className="rounded-xl border border-white bg-white p-3 text-xs font-bold text-slate-700"><span className="block">زر إضافة للسلة</span><div className="mt-2 flex items-center gap-2"><input type="color" value={menuDisplayDraft.cartButtonColor} onChange={event => setMenuCustomizationValue({ cartButtonColor: event.target.value })} className="h-9 w-12 cursor-pointer rounded-lg border border-slate-200 bg-white p-1" aria-label="لون زر إضافة للسلة" /><div className="grid flex-1 grid-cols-3 gap-1">{([{ id: "filled", label: "ممتلئ" }, { id: "soft", label: "ناعم" }, { id: "outline", label: "حدود" }] as const).map(style => <button key={style.id} type="button" onClick={() => setMenuCustomizationValue({ cartButtonStyle: style.id })} aria-pressed={menuDisplayDraft.cartButtonStyle === style.id} className={`rounded-md px-1 py-2 text-[10px] font-black ${menuDisplayDraft.cartButtonStyle === style.id ? "bg-orange-500 text-white" : "bg-slate-100 text-slate-600"}`}>{style.label}</button>)}</div></div></div></div><section className="mt-3 rounded-xl border border-slate-200 bg-slate-900 p-4 text-white" data-menu-dark-colors><div className="flex flex-wrap items-start justify-between gap-2"><div><p className="text-sm font-black">ألوان المنيو والوضع الداكن</p><p className="mt-1 text-[10px] leading-4 text-white/60">تحكم كامل في خلفية المنيو والبطاقات والنصوص والحدود، وتُطبّق فورًا على المنيو العام.</p></div><label className="flex items-center gap-2 text-[11px] font-bold"><input type="checkbox" checked={menuDisplayDraft.darkModeEnabled} onChange={event => setMenuCustomizationValue({ darkModeEnabled: event.target.checked })} className="h-4 w-4 accent-orange-500" />تفعيل الوضع الداكن</label></div><div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{([{ key: "darkBackgroundColor", label: "خلفية داكنة" }, { key: "darkSurfaceColor", label: "سطح البطاقات" }, { key: "darkTextColor", label: "نص الوضع الداكن" }, { key: "menuBorderColor", label: "لون الحدود" }] as const).map(color => <label key={color.key} className="space-y-1 text-[10px] font-bold text-white/80">{color.label}<div className="flex items-center gap-2"><input type="color" value={menuDisplayDraft[color.key]} onChange={event => setMenuColor(color.key, event.target.value)} className="h-9 w-10 cursor-pointer rounded-lg border border-white/20 bg-white/10 p-1" aria-label={color.label} /><Input value={menuDisplayDraft[color.key]} onChange={event => setMenuColor(color.key, event.target.value)} dir="ltr" className="h-9 border-white/20 bg-white/10 font-mono text-xs text-white" /></div></label>)}</div></section><div className="mt-3 rounded-xl border border-orange-200 bg-slate-950 p-3 text-white" data-menu-card-live-preview><div className="flex items-center justify-between gap-2"><div><p className="text-xs font-black">معاينة مباشرة لبطاقة الصنف</p><p className="mt-1 text-[10px] text-white/60">تتغير مع كل اختيار قبل حفظ المنيو.</p></div><span className="rounded-full bg-orange-500/20 px-2 py-1 text-[10px] font-black text-orange-200">مسودة</span></div><div className="mt-3 flex max-w-sm overflow-hidden rounded-xl border border-white/10 bg-white/10" dir="rtl"><div className="w-1/2 shrink-0 bg-gradient-to-br from-orange-200/50 to-white/10" style={{ aspectRatio: menuDisplayDraft.imageRatio === "portrait" ? "4 / 5" : menuDisplayDraft.imageRatio === "landscape" ? "4 / 3" : "1 / 1" }} /><div className="flex min-w-0 flex-1 flex-col justify-between p-2"><strong className={`text-xs font-black text-orange-200 ${menuDisplayDraft.oneLineItemName ? "truncate" : "line-clamp-2"}`}>برغر ناصر المشوي</strong><p className="mt-1 line-clamp-2 text-[10px] text-white/70">شرح مبسط للصنف يظهر تحت السعر.</p><div className="mt-2 flex items-center justify-between"><span className="text-xs font-black">32 ر.س</span><span className="rounded-md bg-orange-500 px-2 py-1 text-[10px] font-bold">+ السلة</span></div></div></div></div></div></section><section className="rounded-xl border border-violet-200 bg-violet-50/50 p-3" data-menu-detail-window-controls><div className="flex flex-wrap items-start justify-between gap-2"><div><p className="text-xs font-black text-slate-800">إعدادات نافذة تفاصيل المنتج</p><p className="mt-1 text-[11px] leading-5 text-slate-500">تحكم مستقل في جهة الفتح، موضع النافذة، حجمها، خلفيتها، الصورة، وأزرار التفاعل. كل تغيير يُحفظ منفردًا ويُطبّق على المنيو العام.</p></div><span className="rounded-full bg-white px-2 py-1 text-[10px] font-black text-violet-700">حفظ مستقل</span></div><div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><label className="text-[11px] font-bold text-slate-700">جهة الفتح<select value={menuDisplayDraft.detailWindow.direction} onChange={event => updateDetailWindowSetting("direction", event.target.value as MenuDetailWindowSettings["direction"])} className="mt-1 h-9 w-full rounded-lg border border-slate-200 bg-white px-2 text-xs"><option value="auto">حسب اللغة</option><option value="right">يمين دائمًا</option><option value="left">يسار دائمًا</option></select></label><label className="text-[11px] font-bold text-slate-700">موضع النافذة<select value={menuDisplayDraft.detailWindow.position} onChange={event => updateDetailWindowSetting("position", event.target.value as MenuDetailWindowSettings["position"])} className="mt-1 h-9 w-full rounded-lg border border-slate-200 bg-white px-2 text-xs"><option value="side">لوحة جانبية</option><option value="bottom">من الأسفل</option><option value="top">من الأعلى</option></select></label><label className="text-[11px] font-bold text-slate-700">العرض<select value={menuDisplayDraft.detailWindow.width} onChange={event => updateDetailWindowSetting("width", event.target.value as MenuDetailWindowSettings["width"])} className="mt-1 h-9 w-full rounded-lg border border-slate-200 bg-white px-2 text-xs"><option value="compact">مختصر</option><option value="wide">واسع</option><option value="full">كامل</option></select></label><label className="text-[11px] font-bold text-slate-700">الارتفاع<select value={menuDisplayDraft.detailWindow.height} onChange={event => updateDetailWindowSetting("height", event.target.value as MenuDetailWindowSettings["height"])} className="mt-1 h-9 w-full rounded-lg border border-slate-200 bg-white px-2 text-xs"><option value="full">ملء الشاشة</option><option value="auto">حسب المحتوى</option></select></label><label className="text-[11px] font-bold text-slate-700">خلفية النافذة<select value={menuDisplayDraft.detailWindow.background} onChange={event => updateDetailWindowSetting("background", event.target.value as MenuDetailWindowSettings["background"])} className="mt-1 h-9 w-full rounded-lg border border-slate-200 bg-white px-2 text-xs"><option value="solid">بيضاء واضحة</option><option value="soft">ناعمة</option><option value="glass">زجاجية</option></select></label><label className="text-[11px] font-bold text-slate-700">تعتيم الخلفية<select value={String(menuDisplayDraft.detailWindow.overlayOpacity)} onChange={event => updateDetailWindowSetting("overlayOpacity", Number(event.target.value) as MenuDetailWindowSettings["overlayOpacity"])} className="mt-1 h-9 w-full rounded-lg border border-slate-200 bg-white px-2 text-xs"><option value="20">خفيف</option><option value="40">متوسط</option><option value="60">واضح</option><option value="80">قوي</option></select></label><label className="text-[11px] font-bold text-slate-700">ملاءمة الصورة<select value={menuDisplayDraft.detailWindow.imageFit} onChange={event => updateDetailWindowSetting("imageFit", event.target.value as MenuDetailWindowSettings["imageFit"])} className="mt-1 h-9 w-full rounded-lg border border-slate-200 bg-white px-2 text-xs"><option value="contain">الصورة كاملة</option><option value="cover">ملء الإطار</option></select></label></div><div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">{([ ["showCloseButton", "إظهار زر الإغلاق"], ["closeOnOutside", "الإغلاق عند الضغط خارجًا"], ["closeOnEscape", "الإغلاق بزر Escape"], ["showQuantityControls", "إظهار التحكم بالكمية"] ] as const).map(([key, label]) => <label key={key} className="flex cursor-pointer items-center gap-2 rounded-lg border border-white bg-white px-3 py-2 text-[11px] font-bold text-slate-700"><input type="checkbox" checked={menuDisplayDraft.detailWindow[key]} onChange={event => updateDetailWindowSetting(key, event.target.checked)} className="h-4 w-4 accent-violet-600" />{label}</label>)}</div><p className="mt-2 text-[10px] font-bold text-violet-800">النافذة الحالية: {menuDisplayDraft.detailWindow.position === "side" ? "جانبية" : menuDisplayDraft.detailWindow.position === "bottom" ? "سفلية" : "علوية"} · {menuDisplayDraft.detailWindow.direction === "auto" ? "حسب اتجاه اللغة" : menuDisplayDraft.detailWindow.direction === "right" ? "يمين" : "يسار"} · {menuDisplayDraft.detailWindow.width === "full" ? "عرض كامل" : menuDisplayDraft.detailWindow.width === "wide" ? "عرض واسع" : "عرض مختصر"}</p></section>
