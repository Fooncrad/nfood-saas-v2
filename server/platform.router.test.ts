...
describe("platform procedures", () => {
  ...
  it("scopes the operational restaurant list to the session restaurant", async () => { const db = await getDb(); if (!db) return; const restaurant = (await db.select({ id: restaurants.id }).from(restaurants).where(eq(restaurants.slug, "nssercafa")).limit(1))[0]; if (!restaurant) return; const caller = appRouter.createCaller(context("user", "restaurant_admin", restaurant.id)); await expect(caller.platform.restaurants()).resolves.toEqual(expect.arrayContaining([expect.objectContaining({ id: restaurant.id })])); const rows = await caller.platform.restaurants(); expect(rows.every((row) => row.id === restaurant.id)).toBe(true); await expect(caller.platform.restaurantRoles({ restaurantId: restaurant.id })).resolves.toBeDefined(); await expect(caller.platform.restaurantPermissions()).resolves.toBeDefined(); });

  it("links plan access to restaurant features and keeps Nasser Cafe fully enabled", async () => { const db = await getDb(); if (!db) return; const nasser = (await db.select({ id: restaurants.id }).from(restaurants).where(eq(restaurants.slug, "nssercafa")).limit(1))[0]; if (!nasser) return; const kdsDefinition = (await db.select({ id: featureDefinitions.id }).from(featureDefinitions).where(eq(featureDefinitions.key, "kds")).limit(1))[0]; if (!kdsDefinition) return; await expect(getFeatureAccess(nasser.id, "kds")).resolves.toEqual(expect.objectContaining({ enabled: true, reason: "enabled" })); const allAccess = await appRouter.createCaller(context("admin")).features.allAccess({ restaurantId: nasser.id }); expect(allAccess.length).toBeGreaterThan(0); expect(allAccess.every((feature) => feature.access.enabled)).toBe(true); const starter = (await db.insert(restaurants).values({ name: `Starter access ${Date.now()}`, slug: `starter-access-${Date.now()}`, barcode: `NFOOD-STARTER-${Date.now()}`, status: "active", plan: "Starter" })); const starterId = Number(starter[0].insertId); try { await expect(getFeatureAccess(starterId, "kds")).resolves.toEqual(expect.objectContaining({ enabled: false, reason: "disabled" })); } finally { await db.delete(restaurantFeatures).where(eq(restaurantFeatures.restaurantId, starterId)); await db.delete(restaurants).where(eq(restaurants.id, starterId)); } });

  it("allows Super Admin to update a feature definition and records the audit event", async () => { const db = await getDb(); if (!db) return; const definition = (await db.select().from(featureDefinitions).limit(1))[0]; if (!definition) return; const admin = appRouter.createCaller(context("admin")); await expect(admin.admin.featureDefinitions()).resolves.toEqual(expect.arrayContaining([expect.objectContaining({ id: definition.id, key: definition.key })])); await expect(admin.admin.updateFeatureDefinition({ id: definition.id, defaultLimit: definition.defaultLimit, isAddOn: definition.isAddOn, addonPrice: definition.addonPrice })).resolves.toEqual(expect.objectContaining({ success: true, id: definition.id })); const logs = await admin.platform.auditLogs(); expect(logs.some((event) => event.action === "feature.definition.update" && event.entityId === String(definition.id))).toBe(true); });

  it("allows Admin to activate and enter a restaurant by id, then cleans up the temporary account", async () => { const db = await getDb(); if (!db) return; const actor = (await db.select({ id: users.id }).from(users).limit(1))[0]; if (!actor) return; const slug = `admin-entry-${Date.now()}`; const created = await db.insert(restaurants).values({ name: "اختبار دخول Admin", slug, barcode: `NFOOD-${Date.now()}`, status: "suspended", plan: "Growth" }); const restaurantId = Number(created[0].insertId); const req = { protocol: "https", headers: {}, cookies: { [TEST_SESSION_COOKIE]: "admin-source-token" } } as TrpcContext["req"]; try { const base = context("admin"); const admin = appRouter.createCaller({ ...base, user: { ...base.user, id: actor.id }, req, res: { clearCookie: () => undefined, cookie: () => undefined } as TrpcContext["res"] }); await admin.admin.updateRestaurant({ id: restaurantId, status: "active" }); const updated = (await db.select({ status: restaurants.status }).from(restaurants).where(eq(restaurants.id, restaurantId)).limit(1))[0]; expect(updated?.status).toBe("active"); const entered = await admin.admin.enterRestaurantAccount({ id: restaurantId }); expect(entered).toEqual(expect.objectContaining({ success: true, restaurantId, role: "restaurant_admin" })); } finally { await db.delete(testAccounts).where(eq(testAccounts.email, `restaurant-${restaurantId}@nfood.local`)); await db.delete(auditLogs).where(eq(auditLogs.restaurantId, restaurantId)); await db.delete(restaurants).where(eq(restaurants.id, restaurantId)); } });
