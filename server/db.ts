...
export function isBranchAcceptingOrders(branch: { status: string; openingTime?: string | null; closingTime?: string | null; operatingWindowsJson?: string | null }, channel: string, now = new Date()) { if (branch.status !== "open") return false; const formatter = new Intl.DateTimeFormat("en-GB", { timeZone: "Asia/Riyadh", weekday: "short", hour: "2-digit", minute: "2-digit", hour12: false }); const parts = formatter.formatToParts(now); const weekday = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].indexOf(parts.find((part) => part.type === "weekday")?.value ?? ""); const currentMinutes = Number(parts.find((part) => part.type === "hour")?.value ?? 0) * 60 + Number(parts.find((part) => part.type === "minute")?.value ?? 0); const windows = parseBranchOperatingWindows(branch.operatingWindowsJson); const candidates = windows.length ? windows.filter((window) => window.dayOfWeek === weekday) : (branch.openingTime && branch.closingTime ? [{ dayOfWeek: weekday, startTime: branch.openingTime, endTime: branch.closingTime }] : null); if (!candidates) return true; return candidates.some((window) => { if (window.channels?.length && !window.channels.includes(channel)) return false; const [startHour, startMinute] = window.startTime.split(":").map(Number); const [endHour, endMinute] = window.endTime.split(":").map(Number); const start = startHour * 60 + startMinute; const end = endHour * 60 + endMinute; return start <= end ? currentMinutes >= start && currentMinutes < end : currentMinutes >= start || currentMinutes < end; }); }
