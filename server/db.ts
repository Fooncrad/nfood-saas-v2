...
export async function createReservationWithTable(input: { restaurantId: number; branchId: number | null; slotId?: number | null; seatingSectionId?: number | null; customerName: string; email?: string | null; phone?: string | null; partySize: number; childrenCount?: number; policyAcceptedAt?: Date | null; reservedFor: Date; durationMinutes?: number; notes?: string | null; createdByUserId?: number | null }) {
  const db = await getDb(); if (!db) throw new Error("Database is not available");
