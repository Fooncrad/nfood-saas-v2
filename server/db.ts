...
export async function createWaiterCall(input: { restaurantId: number; branchId: number; tableName: string; reason: string; customerName?: string | null }) {
  ...
  const created = await db.transaction(async (tx) => {
    ...
    for (const assignment of assignments) {
      const result = await tx.insert(waiterCalls).values({ restaurantId: input.restaurantId, branchId: input.branchId, tableId: table.id, waiterUserId: assignment.waiterUserId, reason: input.reason.trim(), customerName: input.customerName?.trim() || null, status: "active", createdAt: now });
  ...
  return { success: true, tableId: table.id, tableName: table.name, cooldownMinutes, availableAt: new Date(now.getTime() + cooldownMinutes * 60_000), waiters: created };
}
