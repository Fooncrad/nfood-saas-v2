...
export async function getTestAccountByEmail(email: string) { const db = await getDb(); if (!db) return undefined; const normalized = email.toLowerCase(); try { const rows = await db.select().from(testAccounts).where(eq(testAccounts.email, normalized)).limit(1); return rows[0]; } catch (error) { console.warn("[Auth] testAccounts schema is behind; using legacy-compatible account lookup", error instanceof Error ? error.message : error); const rows = await db.execute(sql`SELECT id, email, displayName, role, passwordHash, createdAt FROM testAccounts WHERE email = ${normalized} LIMIT 1`); const raw = (rows as any)?.[0]; const row = Array.isArray(raw) ? raw[0] : undefined; if (!row) return undefined; return { ...row, restaurantId: null, phone: null, permissionsJson: null, isActive: true } as typeof testAccounts.$inferSelect; } }
