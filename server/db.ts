...
export async function listSupportAgents() { const db = await getDb(); if (!db) return []; return db.select({ id: supportAgents.id, userId: supportAgents.userId, isActive: supportAgents.isActive, skillsJson: supportAgents.skillsJson }).from(supportAgents).where(eq(supportAgents.isActive, true)); }
