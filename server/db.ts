...
export async function listAuditLogs(filters?: { restaurantId?: number; actorUserId?: number; actorRole?: string; actorName?: string; action?: string; actions?: string[]; from?: Date; to?: Date; limit?: number }) { const db = await getDb(); if (!db) return []; const conditions = [filters?.restaurantId ? eq(auditLogs.restaurantId, filters.restaurantId) : undefined, filters?.actorUserId ? eq(auditLogs.actorUserId, filters.actorUserId) : undefined, filters?.actorRole ? eq(auditLogs.actorRole, filters.actorRole) : undefined, filters?.actorName ? like(users.name, `%${filters.actorName}%`) : undefined, filters?.action ? eq(auditLogs.action, filters.action) : undefined, filters?.actions?.length ? inArray(auditLogs.action, filters.actions) : undefined, filters?.from ? gte(auditLogs.createdAt, filters.from) : undefined, filters?.to ? lte(auditLogs.createdAt, filters.to) : undefined].filter((condition): condition is NonNullable<typeof condition> => Boolean(condition)); const whereClause = conditions.length > 0 ? and(...conditions) : undefined; return db.select({ id: auditLogs.id, restaurantId: auditLogs.restaurantId, branchId: auditLogs.branchId, actorUserId: auditLogs.actorUserId, actorRole: auditLogs.actorRole, actorName: users.name, action: auditLogs.action, entityType: auditLogs.entityType, entityId: auditLogs.entityId, outcome: auditLogs.outcome, requestId: auditLogs.requestId, metadata: auditLogs.metadata, createdAt: auditLogs.createdAt }).from(auditLogs).leftJoin(users, eq(auditLogs.actorUserId, users.id)).where(whereClause).orderBy(desc(auditLogs.createdAt)).limit(filters?.limit ?? 100); }
