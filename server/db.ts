...
export async function saveDeliveryZone(input: { id?: number; restaurantId: number; branchId?: number | null; name: string; centerLatitude: number; centerLongitude: number; radiusKm: number; deliveryFee: number; minimumOrder: number; isActive: boolean }) { const db = await getDb(); if (!db) throw new Error("Database is not available"); const values = { restaurantId: input.restaurantId, branchId: input.branchId ?? null, name: input.name, centerLatitude: input.centerLatitude.toFixed(7), centerLongitude: input.centerLongitude.toFixed(7), radiusKm: input.radiusKm.toFixed(2), deliveryFee: input.deliveryFee.toFixed(2), minimumOrder: input.minimumOrder.toFixed(2), isActive: input.isActive }; if (input.id) { await db.update(deliveryZones).set({ ...values, updatedAt: new Date() }).where(and(eq(deliveryZones.id, input.id), eq(deliveryZones.restaurantId, input.restaurantId))); return input.id; } const result = await db.insert(deliveryZones).values(values); return Number(result[0].insertId); }
...
export async function calculateDeliveryQuote(input: { restaurantId: number; branchId: number; latitude: number; longitude: number; subtotal: number }) { const zones = await listDeliveryZones(input.restaurantId, input.branchId); const matches = zones.map((zone) => ({ zone, distanceKm: haversineDistanceKm(input.latitude, input.longitude, Number(zone.centerLatitude), Number(zone.centerLongitude)) })).filter(({ zone, distanceKm }) => zone.isActive && distanceKm <= Number(zone.radiusKm) && input.subtotal >= Number(zone.minimumOrder)).sort((a, b) => Number(a.zone.deliveryFee) - Number(b.zone.deliveryFee)); const match = matches[0]; return match ? { available: true as const, fee: Number(match.zone.deliveryFee), minimumOrder: Number(match.zone.minimumOrder), zoneId: match.zone.id, zoneName: match.zone.name, distanceKm: Number(match.distanceKm.toFixed(2)) } : { available: false as const, fee: 0, minimumOrder: 0, zoneId: null, zoneName: null, distanceKm: null }; }
