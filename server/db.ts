...
export async function getOrCreateRestaurantArchiveFolder(restaurantId: number, createdByUserId: number) { const db = await getDb(); if (!db) throw new Error("Database is not available"); const existing = await db.select({ id: mediaFolders.id }).from(mediaFolders).where(and(eq(mediaFolders.scope, "restaurant"), eq(mediaFolders.restaurantId, restaurantId), eq(mediaFolders.name, "Menu Archive"))).limit(1); if (existing[0]) return existing[0].id; const validActor = (await db.select({ id: users.id }).from(users).where(eq(users.id, createdByUserId)).limit(1))[0]?.id ?? (await db.select({ userId: restaurantMembers.userId }).from(restaurantMembers).where(eq(restaurantMembers.restaurantId, restaurantId)).limit(1))[0]?.userId ?? (await db.select({ id: users.id }).from(users).limit(1))[0]?.id; if (!validActor) throw new Error("لا يوجد مستخدم صالح لإنشاء مجلد الأرشيف"); const result = await db.insert(mediaFolders).values({ scope: "restaurant", restaurantId, ownerUserId: null, name: "Menu Archive", createdByUserId: validActor }); return Number(result[0].insertId); }
