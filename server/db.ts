...
export async function upsertReceiptTemplate(input: { restaurantId: number; headerText: string; footerText: string; logoUrl?: string | null; messageTemplatesJson?: string | null; escPosReceiptTemplate?: string | null; escPosKitchenTemplate?: string | null; createdByUserId?: number | null }) { const db = await getDb(); if (!db) throw new Error("Database is not available"); const existing = await getReceiptTemplate(input.restaurantId); if (existing) { await db.update(receiptTemplates).set({ headerText: input.headerText, footerText: input.footerText, logoUrl: input.logoUrl === undefined ? existing.logoUrl : input.logoUrl, messageTemplatesJson: input.messageTemplatesJson === undefined ? existing.messageTemplatesJson : input.messageTemplatesJson, escPosReceiptTemplate: input.escPosReceiptTemplate === undefined ? existing.escPosReceiptTemplate : input.escPosReceiptTemplate, escPosKitchenTemplate: input.escPosKitchenTemplate === undefined ? existing.escPosKitchenTemplate : input.escPosKitchenTemplate, updatedAt: new Date() }).where(eq(receiptTemplates.restaurantId, input.restaurantId)); return existing.id; } const result = await db.insert(receiptTemplates).values({ restaurantId: input.restaurantId, headerText: input.headerText, footerText: input.footerText, logoUrl: input.logoUrl ?? null, messageTemplatesJson: input.messageTemplatesJson ?? null, escPosReceiptTemplate: input.escPosReceiptTemplate ?? null, escPosKitchenTemplate: input.escPosKitchenTemplate ?? null, createdByUserId: input.createdByUserId ?? null }); return Number(result[0].insertId); }
