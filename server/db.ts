...
export const PLATFORM_SETTING_KEYS = ["supportEmail", "supportPhone", "defaultCurrency", "defaultTimezone", "baseDomain", "maintenanceMode", "allowGuestCheckout", "siteLanguage", "availableLanguages", "country", "siteName", "siteLogoUrl", "socialLinks", "copyrightYear", "currencyDisplayMode", "numberFormat", "pricingLayout", "analyticsId", "facebookPixelId", "siteDescription", "homepageContent", "termsOfService", "privacyPolicy", "refundPolicy", "subscriptionTaxRate", "taxNumber", "companyDetails", "bankAccountName", "bankName", "bankAccountNumber", "bankTransferInstructions", "vcardEnabledRoles", "referralReferrerPoints", "referralReferredPoints", "contentImagePrice", "customerStudioLimitBytes", "seoTitle", "seoDescription", "seoKeywords", "seoHashtags", "seoImageUrl", "seoCanonicalUrl", "seoRobots", "googleSearchConsoleVerification", "googleAnalyticsMeasurementId", "googleTagManagerId", "structuredDataJson"] as const;
...
export async function createReservationWithTable(input: { restaurantId: number; branchId: number | null; slotId?: number | null; seatingSectionId?: number | null; customerId?: number | null; customerName: string; email?: string | null; phone?: string | null; partySize: number; childrenCount?: number; policyAcceptedAt?: Date | null; reservedFor: Date; durationMinutes?: number; notes?: string | null; createdByUserId?: number | null; assignedTableId?: number | null }) {
  ...
  return db.transaction(async (tx) => {
    ...
    for (const table of candidates) {
      ...
      const result = await tx.insert(reservations).values({ restaurantId: input.restaurantId, branchId: input.branchId, slotId: input.slotId ?? null, createdByUserId: input.createdByUserId ?? null, customerId: input.customerId ?? null, assignedTableId: table.id, seatingSectionId: input.seatingSectionId ?? table.seatingSectionId ?? null, kind: "reservation", customerName: input.customerName, email: input.email ?? null, phone: input.phone ?? null, partySize: input.partySize, childrenCount: input.childrenCount ?? 0, policyAcceptedAt: input.policyAcceptedAt ?? null, durationMinutes, reservedFor: start, status: "confirmed", notes: input.notes ?? null });
...
export async function getPlatformSettings() {
  const db = await getDb();
  const defaults: Record<PlatformSettingKey, string> = { supportEmail: "", supportPhone: "", defaultCurrency: "SAR", defaultTimezone: "Asia/Riyadh", baseDomain: "", maintenanceMode: "false", allowGuestCheckout: "true", siteLanguage: "ar", availableLanguages: "ar,en,fr", country: "Saudi Arabia", siteName: "NFOOD Restaurant SaaS", siteLogoUrl: "", socialLinks: "", copyrightYear: String(new Date().getFullYear()), currencyDisplayMode: "symbol", numberFormat: "1,000.00", pricingLayout: "style-1", analyticsId: "", facebookPixelId: "", siteDescription: "", homepageContent: "", termsOfService: "", privacyPolicy: "", refundPolicy: "", subscriptionTaxRate: "0", taxNumber: "", companyDetails: "", bankAccountName: "NFOOD", bankName: "", bankAccountNumber: "", bankTransferInstructions: "", vcardEnabledRoles: "customer", referralReferrerPoints: "100", referralReferredPoints: "50", contentImagePrice: "5.00", customerStudioLimitBytes: "104857600", seoTitle: "", seoDescription: "", seoKeywords: "", seoHashtags: "", seoImageUrl: "", seoCanonicalUrl: "", seoRobots: "index,follow", googleSearchConsoleVerification: "", googleAnalyticsMeasurementId: "", googleTagManagerId: "", structuredDataJson: "" };
...
export async function purchaseContentWithWallet(input: { listingId: number; buyerUserId: number; buyerType: "customer"; restaurantId?: number | null }) { const db = await getDb(); if (!db) throw new Error("Database is not available"); return db.transaction(async (tx) => { const row = (await tx.select({ listing: contentListings }).from(contentListings).where(and(eq(contentListings.id, input.listingId), eq(contentListings.status, "published"))).limit(1))[0]; if (!row) throw new Error("المحتوى غير متاح للشراء"); if (row.listing.ownerUserId === input.buyerUserId) throw new Error("لا يمكن شراء المحتوى من صاحبه"); const prior = (await tx.select({ id: contentPurchaseOrders.id }).from(contentPurchaseOrders).where(and(eq(contentPurchaseOrders.buyerUserId, input.buyerUserId), eq(contentPurchaseOrders.status, "approved"), sql`JSON_CONTAINS(${contentPurchaseOrders.itemsJson}, JSON_OBJECT('listingId', ${input.listingId}))`)).limit(1))[0]; if (prior) throw new Error("تم شراء هذا المحتوى مسبقًا"); const amount = Number(row.listing.price); if (!Number.isFinite(amount) || amount <= 0) throw new Error("سعر المحتوى غير صالح"); const buyer = (await tx.select().from(walletAccounts).where(eq(walletAccounts.customerId, input.buyerUserId)).limit(1))[0]; if (!buyer || Number(buyer.balance) < amount) throw new Error("رصيد محفظتك لا يكفي لإتمام الشراء"); const now = new Date(); const orderResult = await tx.insert(contentPurchaseOrders).values({ restaurantId: input.restaurantId ?? null, buyerUserId: input.buyerUserId, buyerType: "customer", paymentSource: "wallet", customerUserId: input.buyerUserId, itemsJson: JSON.stringify([{ listingId: row.listing.id, title: row.listing.title, price: String(row.listing.price), currencyCode: row.listing.currencyCode }]), total: amount.toFixed(2), currencyCode: row.listing.currencyCode, status: "approved", customerName: null, note: "شراء من محفظة المحتوى" }); const orderId = Number(orderResult[0].insertId); const buyerNext = Number(buyer.balance) - amount; await tx.update(walletAccounts).set({ balance: buyerNext.toFixed(2), updatedAt: now }).where(and(eq(walletAccounts.id, buyer.id), gte(walletAccounts.balance, amount.toFixed(2)))); const owner = (await tx.select().from(walletAccounts).where(eq(walletAccounts.customerId, row.listing.ownerUserId)).limit(1))[0]; if (!owner) throw new Error("محفظة صانع المحتوى غير متاحة"); const reward = Number((amount * 0.8).toFixed(2)); const ownerNext = Number(owner.balance) + reward; await tx.update(walletAccounts).set({ balance: ownerNext.toFixed(2), updatedAt: now }).where(eq(walletAccounts.id, owner.id)); await tx.insert(walletTransactions).values([{ walletAccountId: buyer.id, customerId: input.buyerUserId, type: "debit", amount: amount.toFixed(2), balanceAfter: buyerNext.toFixed(2), referenceType: "content_purchase", referenceId: orderId, note: "شراء محتوى Studio من المحفظة", createdAt: now }, { walletAccountId: owner.id, customerId: row.listing.ownerUserId, type: "credit", amount: reward.toFixed(2), balanceAfter: ownerNext.toFixed(2), referenceType: "content_reward", referenceId: orderId, note: "مكافأة بيع محتوى Studio بنسبة 80%", createdAt: now }]); return { orderId, amount: amount.toFixed(2), reward: reward.toFixed(2), buyerBalance: buyerNext.toFixed(2), ownerBalance: ownerNext.toFixed(2) }; }); }
