...
function getDatabaseUrl() {
  ...
  // Hosting dashboards sometimes persist wrapping quotes as part of the value.
  // Strip only one matching outer quote pair; never log credentials.
...
export async function getIntegrationSetting(scope: "platform" | "restaurant", providerKey: string, restaurantId?: number) { const db = await getDb(); if (!db) return null; return (await db.select({ providerKey: integrationSettings.providerKey, status: integrationSettings.status, keyReference: integrationSettings.keyReference, secretCiphertext: integrationSettings.secretCiphertext }).from(integrationSettings).where(and(eq(integrationSettings.scope, scope), eq(integrationSettings.providerKey, providerKey), scope === "restaurant" ? eq(integrationSettings.restaurantId, restaurantId ?? 0) : eq(integrationSettings.scope, "platform"))).limit(1))[0] ?? null; }
...
export async function listMenuCategories(restaurantId: number) { const db = await getDb(); return db ? db.select().from(menuCategories).where(eq(menuCategories.restaurantId, restaurantId)).orderBy(menuCategories.sortOrder, menuCategories.id) : []; }
...
export async function getTestAccountByEmail(email: string) { const db = await getDb(); if (!db) return undefined; const normalized = email.toLowerCase(); try { const rows = await db.select().from(testAccounts).where(eq(testAccounts.email, normalized)).limit(1); return rows[0]; } catch (error) { console.warn("[Auth] testAccounts lookup unavailable", error instanceof Error ? error.message : error); return undefined; } }
