...
export async function saveMyQuickNoteTemplate(userId: number, input: { id?: string; text: string }) { const db = await getDb(); if (!db) throw new Error("Database is not available"); const text = input.text.trim().slice(0, 1000); if (text.length < 2) throw new Error("قالب الملاحظة قصير جدًا"); const preferences = await getUserPreferences(userId); const templates = parseQuickNoteTemplates(preferences?.noteTemplatesJson); const now = Date.now(); const existingIndex = input.id ? templates.findIndex((template) => template.id === input.id) : -1; const duplicateIndex = templates.findIndex((template) => template.text === text && template.id !== input.id); if (duplicateIndex >= 0) return templates; const next = [...templates]; if (existingIndex >= 0) next[existingIndex] = { ...next[existingIndex], text, updatedAt: now }; else next.unshift({ id: nanoid(10), text, updatedAt: now }); const limited = next.slice(0, 8); if (preferences) await db.update(userPreferences).set({ noteTemplatesJson: JSON.stringify(limited), updatedAt: new Date() }).where(eq(userPreferences.userId, userId)); else await db.insert(userPreferences).values({ userId, language: "ar", themeMode: "system", themePreset: "nfood-sunset", noteTemplatesJson: JSON.stringify(limited) }); return limited; }
export async function deleteMyQuickNoteTemplate(userId: number, id: string) { const db = await getDb(); if (!db) throw new Error("Database is not available"); const preferences = await getUserPreferences(userId); const next = parseQuickNoteTemplates(preferences?.noteTemplatesJson).filter((template) => template.id !== id); if (preferences) await db.update(userPreferences).set({ noteTemplatesJson: JSON.stringify(next), updatedAt: new Date() }).where(eq(userPreferences.userId, userId)); return next; }
